...
export function defaultAuditPath(): string {
  return process.env.STANDALONE_IOS_AUDIT_PATH
    ?? join(process.env.STANDALONE_SKILL_STATE ?? resolve(import.meta.dir, '../../../../..', '.agents', 'skill-state'), 'security', 'ios-qa-audit.jsonl');
}

export function defaultAttemptsPath(): string {
  return process.env.STANDALONE_IOS_ATTEMPTS_PATH
    ?? join(process.env.STANDALONE_SKILL_STATE ?? resolve(import.meta.dir, '../../../../..', '.agents', 'skill-state'), 'security', 'attempts.jsonl');
}
...
async function loadDeviceSalt(): Promise<string> {
  ...
  const path = join(process.env.STANDALONE_SKILL_STATE ?? resolve(import.meta.dir, '../../../../..', '.agents', 'skill-state'), 'security', 'device-salt');
  try {
