...
//
// Invoked from bin/standalone-ios-qa-mint.
...
function printHelp() {
  const help = `standalone-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  standalone-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  standalone-ios-qa-mint revoke --remote <identity>
  standalone-ios-qa-mint list
...
EXAMPLES
  standalone-ios-qa-mint grant --remote 'alice@example.com' --capability interact
  standalone-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  standalone-ios-qa-mint revoke --remote 'alice@example.com'
  standalone-ios-qa-mint list

The allowlist lives at <project>/.agents/skill-state/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`standalone-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}
