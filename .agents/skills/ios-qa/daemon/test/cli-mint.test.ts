// CLI tests for standalone-ios-qa-mint. Invokes the bash launcher end-to-end
...
// so the project's real .agents/skill-state/ios-qa-allowlist.json is untouched.
...
const MINT_BIN = join(ROOT, 'bin', 'standalone-ios-qa-mint');
const DAEMON_BIN = join(ROOT, 'bin', 'standalone-ios-qa-daemon');

function runMint(args: string[]) {
  return spawnSync(MINT_BIN, args, { stdio: 'pipe', encoding: 'utf-8' });
}

describe('bin/standalone-ios-qa-mint launcher', () => {
  let tmpDir: string;
  let listPath: string;
  ...
  test('--help prints usage without touching allowlist', () => {
    ...
    expect(r.stdout).toContain('standalone-ios-qa-mint');
...
describe('bin/standalone-ios-qa-daemon launcher', () => {
