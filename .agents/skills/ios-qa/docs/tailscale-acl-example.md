...
- **Auth:** Tailscale identity validation via the local `tailscaled` socket
  ...
  `<project>/.agents/skill-state/ios-qa-allowlist.json` is the single source of truth for who can
  do what.
...
standalone-ios-qa-mint --remote ci@example.com --capability mutate --ttl 1h
...
row to `<project>/.agents/skill-state/security/ios-qa-audit.jsonl`:
...
allowlisted, rate limit hit) write to `<project>/.agents/skill-state/security/attempts.jsonl`.
...
| `403 identity_not_allowed` | identity missing from allowlist | Owner mint: `standalone-ios-qa-mint --remote <identity>` |
