...
description: Kirole 仓库自带诊断工具地图：eink-simulator、虚拟硬件测试、frame trace、keep-alive、PromptDebugger、fastlane status、ASC 校验脚本。需要观察现场时查这份。NOT for 排查流程本身（各 runbook）——本页只讲工具在哪、怎么开、有什么坑。
---
...
- **PromptDebugger（+FAB）**：看实际组装的 prompt。注意门控历史——prompt 注入面必须
  ...
  mock context 会保留 customCompanion（`67cf7e1`），所以自定义伴侣的 prompt 也能在此调试。
