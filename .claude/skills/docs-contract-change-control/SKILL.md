...
description: 修改 docs/ 下硬件契约文档（BLE 协议规格、固件功能规格、联调指南）的流程：版本号、§ 交叉引用、飞书同步、禁止镜像。NOT for CLAUDE.md/AGENTS.md 等 App 内部文档，或动真字节的改动（先去 ble-wire-change-control）。
---
...
- `docs/positioning-narrative.md` 等产品叙事 → 归 `product-scope-contract` 管辖
...
1. 版本号递增（当前 v2.5.x 系列），文档头部"状态"段落更新;
2. §1.3 修订历史表加一行——**纯文档变更也要记版本并注明"wire 不变"**
   （范例：v2.5.13 字体映射、v2.5.15 面板态拍板，都是纯文档版）；
