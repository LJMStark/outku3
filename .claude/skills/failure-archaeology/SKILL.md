...
description: Kirole 仓库事故编年史：每次踩坑的时间线、根因、修复 commit、固化产物。用于防复发、理解现有防线为什么存在。NOT for 排查当前故障（去对应 runbook）——这是记忆库不是流程图。
---
...
## 事故 #1：TestFlight 543 —— 反馈环静默断裂三周（最贵）

- **时间线**：`1b8aa1c`（05-11）build 543 上线 → 持续发版三周，fastlane 每次 "Done" →
  外部测试者与公共链接始终停在 543 → 误诊出口合规（`c82ba9e`）→ 真因 `NO_SUBMISSION`：
  分发从未提交 Beta App Review，公共链接只发最后一个 APPROVED build → `5c49e36` 根治，
  `a2a376b` 纠正误诊文档。
- **为什么最贵**：不是改坏了什么，而是**不知道自己在坑里**——三周的迭代对外部用户为零，
  决策建立在"用户在用新版"的假象上。
...
且该次运行经历过 502 中断）→ 全部还原 → 当晚 `658a2fb` 产品**正式**删除（sensitiveBoundary
成为唯一开关）。同一删除，agent 自作主张=事故、人拍板=决策。
姊妹案例：`225e954`（04-07）"radical simplification" 删掉人格描述后当天 restore。
...
## 事故 #8：静默失败家族（06-12 批次）

审计发现六条错误被吞的路径（`95e0354` 补日志）；同族修复：usage state 读失败时**别用
零值覆盖**（`d8eea29`）、瞬态 session-restore 错误**别清 token**（`fbc2ae0`）、
