...
| 17 | 未来/过期/无日期任务不出现在硬件上（`0x23` 不发它们） | **2026-08-04 客户拍板**：设备任务库只含当天（dueDate 当天 ∪ 手动设为今天）；条数仍不设上限。审计报"设备漏任务/日期被截断"先查本条 | ADR 0029（supersedes 0001/0002）、BLE v2.16.0 §4.22、`TaskItem.isEligibleForHardwareTaskLibrary` |
| 18 | 跨零点后设备短暂显示"昨天的任务库" | 有意：设备不自行按日期过滤，旧 committed 库沿用到 App 推来新日重算版（过期显示优于空白/半包） | ADR 0029 + ADR 0006 补条、BLE §4.22 规则 11 |
