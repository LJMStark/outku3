---
name: kirole-atlas
description: Kirole 仓库特定技能的总索引/路由表：第一次进本仓库、不确定该读哪份技能、或要维护这批文档时用。按"你正要做的事"（改字节/删资产/测试忽红忽绿/发版/审 agent 报告…）查该开哪份。NOT for 获取知识本体——本文只做路由，答案在目标文档里。
---

# Kirole 技能地图（Atlas）

本仓库有 15 份仓库特定技能文档（含本份），全部由 2026-07 的仓库考古产出：内容来自 commit 历史、事故记录、
memory 沉淀——不复述 CLAUDE.md / AGENTS.md 的叙述本体（契约/验收类会把真源规则蒸馏成
可执行清单，并署名出处）。CLAUDE.md 讲"这个仓库是什么"，这批文档讲
"在这个仓库里怎么不受伤"。

## 什么时候不该用本文档

- 你已经知道要找哪个主题 → 直接打开那份，别绕路。
- 你要了解产品定位、构建命令、架构地图 → 读 `CLAUDE.md` 与 `AGENTS.md`，那是真源，本批文档只做蒸馏与补充。
