...
description: LLM 管线安全与输出契约：PromptSanitizer 注入点（防 prompt 注入/越狱）、输出字节预算、错误文本隔离、provider fallback、custom-prompt 审计结论。碰 OpenAIService/CompanionTextService/DayPackGenerator/prompt 组装前必读。NOT for 文风调优（AGENTS.md IP 规格是真源）或 OAuth/Keychain 安全。
---
...
3. **custom-prompt 已审计（build 580，memory `project_custom_prompt_audit_580`）**，结论直接引用、别重审：
   ...
   - 已知推迟项——**别重报**；完整清单以 `subagent-output-audit` 流程 2 为准（单一维护点，此处不复述）；
...
- custom companion 激活时（`UserProfile.customCompanionId != nil`），prompt 走
  ...
  （`306b8f6` / `1c0dcb4` 补过 DayPack 与 TaskInPage 两条线）。
- 对话缓存指纹以 custom id + updatedAt 版本化，任何人格字段变更（含 voice）都会失效
  （`e5a29d4` 用 updatedAt 版本化、`7ab9703` 用 bitPattern 处理亚秒），
  改人格字段时想着指纹会不会漏更新。
