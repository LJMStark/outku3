...
description: 产品红线契约：评审新功能/需求/审计建议时判断"该不该做"。四红线=不待办化（拒 AI 任务拆解/详情步骤/催办提醒）、不做多入口（Watch/Mac/家庭共享）、单设备模型、英文 UI。含执法史（streak 连击、进化系统被删）与灰区判例。NOT for 技术实现方案评审；陪伴向增强（俳句/亲密度/场景解锁）是欢迎的，不是挡箭牌。
---
...
## 四条红线（前三条 codify 于 AGENTS.md §5，`ecde626`；英文 UI codify 于 CLAUDE.md Interaction Rules #4，`d59e414`）
...
- `docs-contract-change-control` — 硬件契约文档的变更流程（positioning-narrative.md **不**走它的版本/飞书流程：按普通 markdown 修改，方向受本契约约束）
