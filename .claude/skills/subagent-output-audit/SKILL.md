...
description: subagent（code-simplifier/refactor-cleaner 等）跑完、或 502 中断后接手、或收到 review/audit 报告后的强制验证：先 git diff 审全部改动再动手。曾有 agent 把整个功能当"过度设计"删掉。误报对照 intentional-behaviors-contract。NOT for 人手写的小 diff，或用户已明确拍板的删除。
---
...
> 玩味之处：当晚产品**真的**拍板收敛了它（`658a2fb`：CustomCompanion 层的 roastModeEnabled
> 布尔由 sensitiveBoundary 取代；注意 `OnboardingProfile.customCompanionRoast` 与 onboarding
> 的 Roast Mode 开关**仍在役**，别再把它们当死代码清）——但那是**人拍的板**。
> 同一个删除，agent 自作主张=事故，人拍板=决策。
>
> 同类事故：`3723af7`（04-07）——"radically simplify companion prompt" 把 Role/Tone/Directives
> 人格描述简没了，当天 `225e954` 不得不 restore。"激进简化"是 agent 的系统性倾向。
