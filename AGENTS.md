...
## 2. Project Context
- **Hardware BLE 1.3.1 source of truth (flag-day with firmware)**: read `docs/专注状态重连协议变更_Ver_1_3_1.md` then `docs/Kirole_BLE协议命令字节表_专注重连协议更新_Ver_1_3_1.md`. Arbitration table still lives in `docs/Kirole_专注状态重连_App对接说明_Ver_1_3_0.md` §9. Repo-wide spec is `docs/BLE通信协议规格文档.md` (v2.13.3, §4.23 / §5.15 aligned to Ver 1.3.1 directly: only `RESULT=COMMITTED` unlocks `0x14`, no `BEGIN/COMMIT` while focus is active, batch `0x10/0x11/0x12` records are v2 `19+N` / `23+N`). The `-v2.13.1补记` / `-v2.13.2补记` files are historical records, already folded in. The hardware 1.3.1 byte table §5.15 leftover v2.9 batch rows are not App contract; use main-spec §5.15 plus live §5.3–5.5. Remaining questions: `docs/专注重连-与硬件协商项_Ver_1_3_1.md`. Do not unlock `0x14` until `RESULT=COMMITTED`, and do not `COMMIT` while focus is still active.
...
Kirole has exactly **two distributed binaries** reaching **three audiences**. Local `Debug` builds are development tools, not a distribution channel. Build configuration encodes the *code boundary* (what is compiled in); the audience is an App Store Connect property; secrets and switches are build *inputs*, never a third configuration.

| Audience | Binary | Purpose | Required build behavior |
|----------|--------|---------|-------------------------|
| **Internal TestFlight** (`fastlane ios internal`) | `Kirole-Internal` / `InternalRelease` | Product acceptance and hardware/firmware debugging by the internal team | Release-optimized build with all internal/debug tools enabled; assigned to the manual **Kirole Hardware Internal** group only — never to external groups |
| **External TestFlight** (`fastlane ios external`) | `Kirole-AppStore` / `AppStoreRelease` | Real-device verification of the customer candidate by external testers | The **same configuration and archive** as the App Store candidate; internal tools compiled out; distributed to external groups + Beta App Review |
| **App Store** (promote the external build; `fastlane ios appstore` only for a fresh archive) | `Kirole-AppStore` / `AppStoreRelease` | Customer distribution | Release-optimized production build with internal/debug UI, behavior, logging, factory commands, and test shortcuts compiled out. Submit the build number external testers verified instead of archiving a third binary |
...
6. Build the customer candidate from a tagged commit with `fastlane ios external` and let external testers verify it on real devices; the App Store submission promotes that same build number. Internal TestFlight acceptance is product evidence, but it is not proof that the differently compiled `AppStoreRelease` binary works; run the production gate tests and real-device smoke test on the external build.
...
- **Implemented 2026-09-03 (lane split + firmware-readiness switch)**: `fastlane ios internal` archives `Kirole-Internal` and assigns it to the manual **Kirole Hardware Internal** group only; `fastlane ios external` runs the secure-channel gate + `scripts/verify-release-boundary.sh`, archives `Kirole-AppStore`, and distributes to every external group + Beta App Review; `fastlane ios appstore` uploads a `Kirole-AppStore` binary only (prefer promoting the external build). Builds ≤ 656 in the external **kirole** group are `InternalRelease` binaries from the old combined `release` lane — the pending "Internal Only" decision is closed. `BLE_SECURE_CHANNEL_ENABLED = 0` (see Compile-Time Debug Boundary): all channels ship plaintext BLE until the hardware team receives, provisions, and confirms the secret; **App Store candidates 651 and 655 were archived in `.secure` mode with a secret the firmware never received and must not be released** — the next customer candidate comes from the switch-0 `external` pipeline.
- **Implemented 2026-08-17 (App Store wash)**: internal Settings / Focus Debug UI lives in `Kirole/Internal/` under `#if KIROLE_INTERNAL` and is injected via `InternalToolsViews`. `showsHardwareDebugTools` is off unless the Internal app shell calls `enableInternalHardwareChannel()`. MVP still forces BLE keep-alive on the customer channel (no Settings toggle). `scripts/verify-release-boundary.sh` also asserts the internal-tool UI strings are present in InternalRelease and absent from AppStoreRelease. AppStoreRelease device/archive builds, `fastlane ios external` and `fastlane ios appstore` fail closed on an empty `BLE_SHARED_SECRET` only when `BLE_SECURE_CHANNEL_ENABLED = 1`.
- **Cleared 2026-09-08**: the app passed review and shipped. Screenshots, store copy, the privacy questionnaire, WeatherKit attribution and hardware-claim evidence are all done; the submission record is archived under `docs/app-store/2026-09-07/` (screenshot PNGs are deliberately untracked — `SHA256SUMS` is, so the local/Feishu originals stay verifiable).
- **Still open (not blocking)**: `0x19` / `0x1C` encoders remain in `KiroleFeature` (no user-facing strings; symbol-level removal needs a separate package product). Prompt debugger stays Debug-only.
...
**The rapid-iteration phase ended on 2026-09-08, when the app passed review and shipped to the App Store.** The previous policy — treat `LocalStorage` / `UserDefaults` / on-device JSON as disposable and reset local data instead of migrating — named its own expiry: it held "only until hardware integration, shared staging data, TestFlight, or external users depend on them." External users now do. The boundary below is that explicit documentation.

- **Never reset or drop a shipped user's local data.** Tasks, events, pet state, focus history and gamify counters on a customer device are the only copy; `SyncManager`/Supabase does not back all of them up. A schema change that fails to decode does not surface an error — it silently returns nothing and the user's content is gone.
- **New fields on persisted models must be Optional.** `CalendarEvent`, `TaskItem`, `Pet`, `FocusSession` and everything under `LocalStorage.Files` are decoded from JSON written by older builds. Swift's synthesized `Codable` makes an added Optional field backward compatible for free (a missing key decodes to `nil`), so **Optional-only means no migration code is needed**. A non-Optional addition throws `keyNotFound` and takes the whole file with it.
- **Renaming or removing a persisted field is a breaking change.** Keep the old key decodable (`CodingKeys`, or decode-both-then-prefer) for at least one shipped release before dropping it. Do not "clean up" a field name in the same change that ships new behaviour.
- **Verify against old data, not just new.** A test that encodes and decodes with the current model proves nothing about a file written by the previous release. Add a fixture holding the previous JSON shape when you change a persisted model.
- BLE stays a flag-day firmware contract, unchanged by this shift: Ver 1.3.1 专注重连 + Schedule v2 — `0x14` / Enter / Complete / Skip / `FOCUS_STATE` / `FOCUS_RESOLVE` + `RESULT` / `0x03`. Do not add compatibility branches for pre-v2 frames. `FOCUS_RESOLVE` unlocks only on matching `RESULT/COMMITTED`. Active focus must not send OfflineSync `COMMIT`.
- Removing an obsolete compatibility shim is still encouraged — but only once no shipped build writes the shape it covers, which is now a release-history question rather than a free cleanup.
...
3. ~~**External sync skips companion presentation refresh.**~~ **RESOLVED 2026-05-07**: extracted shared `applyPostSyncHooks()` private helper in `AppState+Sync.swift`; the Apple and Google sync paths call it after applying data.
...
6. ~~**`editTask` / `editEvent` embed external sync logic.**~~ **RESOLVED 2026-05-07**: extracted `Core/Services/ExternalSyncDispatcher.swift` (a `@MainActor enum`) holding the Apple/Google task and event write routes. `+Actions` contains only AppState state mutations and dispatcher calls.
