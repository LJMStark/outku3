...
- **BLE for custom avatars (protocol v2.7)**: 0x15 `customAvatarFrame` (App→Device) is a staging-only payload: `subVersion(0x04) | OperationID | AvatarID | FileLength | FileCRC32 | KRI v1 file bytes`. The old PNG (`0x02`) and anonymous KRI (`0x03`) send paths are removed. `AvatarImageProcessor` aspect-fits into 800×700 (no square crop, never upscales); `KRIEncoder` converts the persisted PNG to straight-alpha BGRA KRI at send time. 0x22 `avatarControl` handles `commit / eraseExact / eraseAll / query / abort` and `staged / committed / erased / state / aborted` results. The App keeps the old companion active until a matching `committed` result arrives. One persisted `PendingCustomAvatarOperation` supports query-based recovery after disconnect or process death; retries restart from chunk zero, with no resumable upload or background-transfer promise.
