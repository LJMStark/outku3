...
- **BLE for custom avatars (protocol v2.5.24)**: 0x15 `customAvatarFrame` (App→Device), payload `subVersion(1B, 0x02) | PNG file bytes`. `AvatarImageProcessor` aspect-fits into 800×700 (no square crop, never upscales), 8-bit sRGB `pngData()`, ×0.9 shrink loop to ≤1MiB; Spectra-6 quantization now happens firmware-side (the app-side 4bpp chain — `EInkColor`, `encodePixelData` — was deleted). Rides the 11-byte chunk header (Seq/Total 2B BE, ≤65535 chunks; worst case ~2093 chunks ≈1-2 min). Push guarded by a PNG-signature check that drops stale pre-PNG assets.
