...
- Fastlane lanes are split: `release` archives `Kirole-Internal` for the TestFlight hardware-acceptance channel (still distributes to the external **kirole** group for tester continuity — moving it to TestFlight Internal Only is a pending workflow decision); `appstore` runs the boundary gate, archives `Kirole-AppStore`, and uploads the binary without any TestFlight distribution.
- **Still open (blocking submission)**: internal tools (Wi-Fi PC Debug, BLE Keep Alive, Focus Debug, factory commands, keep-alive default…) are still compiled into `KiroleFeature` for every configuration, and `AppBuildEnvironment.showsHardwareDebugTools` is still hardcoded `true`. Until each tool is migrated behind the app-target boundary (with paired presence/absence gate tests) and BLE security fails closed, an `AppStoreRelease` archive is still not safe to submit.
