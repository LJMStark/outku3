...
- **BLE for custom avatars (protocol v2.6.1)**: 0x15 `customAvatarFrame` (App→Device), default payload `subVersion(1B, 0x03) | KRI v1 file bytes`; `0x02 | PNG` remains a firmware-debug fallback. `AvatarImageProcessor` aspect-fits into 800×700 (no square crop, never upscales) and keeps the persisted PNG ≤1MiB; `KRIEncoder` converts it to straight-alpha BGRA KRI at send time. Both formats use the 11-byte chunk header (Seq/Total 2B BE, ≤65535 chunks); worst-case KRI is 2,240,013B including SubVersion, ~4472 chunks / 4-5 min at 501B per chunk. A KRI transport write stays pending until DeviceWake `AvatarCRC32` confirms firmware persistence; unconfirmed frames keep the existing back-off retry schedule.
