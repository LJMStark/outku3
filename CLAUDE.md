...
- **Google, Apple, and Outlook coexist** (product decision 2026-09-07, `267c4e5`). Connecting one source must not disconnect another. Duplicates are collapsed at read time, not prevented at connect time: `AppState.presentationEvents` (`CalendarEventPresentation`) hides an Apple mirror of a Google event — keyed strictly on `iCalendarUID`, never on a title/time guess — while keeping both original records so edits still route through their own provider. That projection is what feeds the timeline, companion context **and** the frozen hardware datasets, so mirrors do not consume `ScheduleV2Codec.maxEvents` slots. **Outlook and same-provider copies are exempt from the fold** and do compete for the 8-event budget; that is a capacity trade-off, not a reason to restore provider exclusivity. `AppleCalendarSelectionSheet` shows a manual deselect reminder on top of this, only while Google Calendar is connected.
