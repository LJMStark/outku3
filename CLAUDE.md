...
**Pet 页面布局是设计内容（客户需求，勿动）**：上半部分显示宠物形象，下半部分是任务列表 UI（Tasks Today / Upcoming / No Due Dates，含 checkbox / Edit / Delete）。这是刻意的产品设计，不是"待办增强"，不应被当作与定位冲突的问题处理。
...
- Key subsystems (detail in AGENTS.md §3): Timeline (`TimelineDataSource` + `HomeView`), 14-screen onboarding (`OnboardingState`), Pet/Companion IP system (3 IP characters Joy/Silas/Nova × 5 stages × 5 moods × 4 `Pet.Scene` contexts for in-app pet composition — Indoor/Outdoor/Night/Work), Hardware `DisplayScene` (3 scenes: harbor/forest/nightCity, independent from `Pet.Scene`, unlocked by energy bottles but **user must manually select in Settings → Scenes to push to E-ink** — no auto-apply), Companion text pipeline (`DayPackGenerator` → `CompanionTextService` → `OpenAIService`), BLE sync (`BLEPacketizer`/`BLESyncCoordinator`), Focus mode (`FocusSessionService` + `DisplayScene`), Supabase persistence.
