...
//   0x15 customAvatarFrame 用户自定义伴侣头像（默认 SubVersion 0x03 | KRI ≤2,240,012B；0x02 | PNG ≤1MiB 为联调回退，均 ≤800×700 保比例）
...
public enum BLEDataType: UInt8, Sendable {
    ...
    /// v2.6.1 起默认 payload = 1B subVersion(0x03) | KRI v1 文件字节（文件头自描述宽高，
    /// ≤2,240,012B，固件免 PNG 解码直接交 LVGL）；旧 subVersion(0x02) | PNG（≤1MiB，
    /// v2.5.24 引入）仅剩联调回退通道（`BLEService.avatarKRIPushEnabled` 切 OFF），
