...
public actor OpenAIService {
    ...
    /// Parses the classifier reply ("2,5,1" — tolerant of stray spaces/newlines/labels) into
    /// categories. Throws `malformedResponse` when the digit count does not match the input
    /// count or any digit is outside 1-6 (0/unknown is never a valid model answer).
