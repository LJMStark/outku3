...
public final class BLESyncCoordinator {
    ...
    func reconcileTaskListSnapshotInventory(
        ...
        do {
            switch inventory {
            case .missing:
                // Device lost its snapshot version (RAM-only, power cycle cleared NVS).
                // Clear App's entire delivery state so the next device-triggered 0x1B will carry
                // a fresh random epoch (generated by nextTaskListSnapshotVersion). Return true to
                // trigger a priority sync, which refreshes the display and task library data —
                // 0x1B itself is NOT sent by performSync; it fires only on Complete/Skip/RequestRefresh.
                try await localStorage.clearTaskListSnapshotDeliveryState(for: destinationID)
                return true
            case let .committed(version):
                // Device reports a committed version from NVS. Compare with App's record:
                // - If App has no record or a different version, clear any stale delivery state
                //   (reservation/frozenResponses/attempted) from a pre-restart transaction, then
                //   adopt the device-reported version as the new clean baseline.
                // - If versions already match, no action needed (already aligned).
                let appVersion = try? await localStorage.loadTaskListSnapshotVersion(for: destinationID)
                if appVersion == nil || appVersion != version {
                    try await localStorage.clearTaskListSnapshotDeliveryState(for: destinationID)
                    try await localStorage.saveTaskListSnapshotVersion(version, for: destinationID)
                }
                return false
            }
        } catch {
