...
struct EventCategoryTests {
    ...
    func generatorTagsEventsWithoutAI() async {
        ...
        let events = [
            ...
            CalendarEvent(
                id: "cat-description-deadline", title: "Q3 filing",
                startTime: calendar.date(bySettingHour: 18, minute: 0, second: 0, of: today)!,
                endTime: calendar.date(bySettingHour: 18, minute: 30, second: 0, of: today)!,
        ...
        #expect(dayPack.events.map(\.category) == [.meetings, .wellness, .admin, .deadline])
    }
