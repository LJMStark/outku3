...
@MainActor
@Suite("Focus Debug Timeline", .serialized)
struct FocusDebugTimelineTests {
    ...
    private func withDebugChannel(_ body: () async -> Void) async {
        await SharedPersistenceTestLock.shared.withLock {
            AppBuildEnvironment.enableInternalHardwareChannel()
            defer { AppBuildEnvironment.resetInternalHardwareChannelForTesting() }
            await body()
        }
    }
