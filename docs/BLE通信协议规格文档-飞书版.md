...
> **飞书使用说明：** 本文件为「飞书专用版」，已移除飞书不支持的页内锚点链接、删除线，并去掉表格单元格内的反引号代码标记，避免长表格在飞书散架。建议用飞书「导入 Markdown 文件」导入（云文档 → 新建 → 导入 → 选本文件），表格与代码块会转成飞书原生块；直接复制粘贴到已有富文本文档仍可能让长表格散架。源文件（GitHub / IDE 用，请勿在飞书场景使用）：docs/BLE通信协议规格文档.md。
