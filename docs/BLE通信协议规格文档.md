...
| 1      | TasksTotal          | 1 byte      | -          | 总项数 = 全部任务 + **全部**今日日程（clamp 0-255；v2.6.0 修正——4 个日程过了 1 个显示 1/4 而非 1/1，进度条呈真实中间态） |
...
| 4      | AvatarState  | 1 byte | v2.6.0：`0x00`=本地**无**自定义头像图 / `0x01`=有（已持久化的 0x15 PNG） |
| 5      | AvatarCRC32  | 4 bytes | v2.6.0：已持久化 0x15 PNG 文件字节的 **CRC-32/IEEE**（Big Endian；AvatarState=0 时填 0）。固件**存图时计算一次并缓存**，勿每次唤醒重算 1MiB |
