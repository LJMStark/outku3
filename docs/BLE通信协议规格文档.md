...
> ### ⚠️ 现状（2026-09-03 更正）：App 当前**不发送**本帧，本节描述的强制屏障**不生效**
>
> **本条只针对 App→Device 方向的 `0x20 EventLogRequest`。** 同一字节在 Device→App 方向是 `0x20 RequestRefresh`（§5.7），**完全不受影响、仍然必需**——它是专注会话期间唤醒被 iOS 挂起的 App 的唯一可靠通道，固件必须继续周期发送。方向双义见 §2.4。
>
> App 自 2026-08-13 起改由 **`0x25 OfflineSync` 的 `OP_BATCH`** 完成离线补传，`0x20` 的调用点在同一次改动中被移除，此后从未发出过。因此：
>
> - **v2.16.0 起本帧被描述为「每个连接的强制前置握手」、「收到并处理完 `0x21` 前不下发任何显示数据」、「15 秒未完成即判连接失败并主动断开」——这套行为当前完全不存在。** 固件即使实现了 `0x20` 接收与 `0x21` 回传，也不会被触发。
> - **固件不必为等待 `0x20` 而保留任何超时或屏障逻辑。**
>
> **`0x25 OP_BATCH` 已覆盖本帧的能力（2026-09-03 真机验证）**：`OfflineSyncOperationRecord` 携带 `EventType` + **原始 payload 原文**，App 侧走与 `0x21` 相同的 `EventLog` 解码器，故可承载任意事件类型。实测：杀掉 App → 在设备上离线完成一个任务 → 重连，`enter_task_in`(op=5) 与 `complete_task`(op=6) 均完整补传、带设备时间戳，本轮同步正常提交。
>
> **本帧是否正式废弃，待与硬件团队确认后再定稿**——协议由硬件团队主导（字节表 Ver 1.3.1 §4.13 仍保留本帧定义），本节只如实记录 App 侧现状，不单方面宣布废弃。在确认前**不要**依据旧描述把 `0x20` 发送与 `0x21` 屏障"修回来"：固件是否应答 `0x20` 尚未验证，贸然恢复 fail-closed 屏障可能造成连上即断的重连循环。
...
bit5...bit7 保留；固件必须发送 0，App 忽略这些保留位。当前 App 遇到 `transactionOpen=1` 时先按 CurrentSyncID 发 ABORT，再发第二次 QUERY；如果 `transactionOpen=1` 但 CurrentSyncID=0，整轮失败。遇到 `operationOverflow=1` 时，App 仍处理并 ACK 尚存的 OP_BATCH；若设备未同时要求完整同步，则不再发 BEGIN，避免用无法还原的 App 快照覆盖设备状态。`needsFullSync=1`、`dataValid=0` 或 `ValidUntil` 已过期三者任一成立即视为**设备要求完整同步**：裁决完成且设备处于 idle 后，App 必须进入 §4.23.4 步骤 8–9 做完整状态核对并 COMMIT（硬件 Ver 1.3.1：队列溢出时同时置 `OperationOverflow+NeedsFullSync`，即"溢出 → 完整状态核对"）；活动专注期间照样延后到退出专注后的下一轮。
...
7. 如果最终 STATE 的 `operationOverflow=1` 且设备**未**要求完整同步（`needsFullSync=0`、`dataValid=1`、`ValidUntil` 未过期），App 在处理并 ACK 尚存操作（以及必要的 FOCUS_RESOLVE）后结束本轮，不发 BEGIN。设备要求完整同步时（硬件 Ver 1.3.1 溢出即同时置 `NeedsFullSync`）继续步骤 8–9 做完整状态核对；App 自身无新数据且设备未要求完整同步时同样不发 BEGIN。
...
> **⚠️ 现状（2026-09-03）：本帧当前不会被触发。** 它是 `0x20 EventLogRequest` 的应答，而 App 自 2026-08-13 起不再发送 `0x20`，离线补传改由 `0x25 OP_BATCH` 承担（已真机验证）。固件保留实现无害，但不必为它预留超时或屏障。完整说明与"不要贸然修回来"的理由见 **§4.13 顶部的现状说明**。
...
| 权限被拒绝 | 显示设置跳转引导 |
| 未找到设备 | 重试扫描，显示"未找到设备" |
| 连接建立失败 | App 在当前同步或用户触发轮次内重试；每轮连接建立总尝试上限为 10 次（含首次），达到上限后结束本轮，等待下一个同步窗口或用户再次操作 |
| 意外断开 | 若已启用则自动重连：采用 CoreBluetooth pending connection（`retrievePeripherals` 取回已知设备 + 不超时 connect），不启动扫描，也不受每轮连接建立 10 次上限约束；等待设备回到范围后自动重连 |

**连接建立重试是 App 策略，不改变 BLE 线协议或固件行为**：单次连接建立失败后的取消只清理本次尝试；本轮仍可继续下一次尝试，直到总计 10 次。达到上限后结束本轮，固件正常保持广播即可。

**主动断开不重连**：App 主动断开（sync 收尾、用户点击断开、后台任务到期）**不会**触发自动重连，会等待下一个同步窗口或用户操作。意外断开使用另一条 CoreBluetooth pending connection 路径：不设超时、不扫描，设备回到范围后由系统继续连接。
