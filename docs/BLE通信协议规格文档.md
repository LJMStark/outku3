...
**版本:** v2.18.0
**更新日期:** 2026-08-08
**状态:** v2.18.0 修复 `0x1B TaskListSnapshotAck` 在设备重启后版本错位的根因。本版对固件的增量要求：① NVS 持久化 `0x1B` 快照版本 12B；② `DeviceWake(0x30)` 保留原 42B 前缀不变，payload 追加 9B 快照库存字段至 51B。这是明确的前向兼容扩展：App 必须按实际长度分支解析 42B/51B。

**v2.17.0 回顾（非本版增量，wire 不变）：**
...
**从 Build 640 / BLE v2.10.1 升级的最小分类：**

| 需求 | Build 640 已有信息 | 缺失信息 | 选择 | 理由 |
|------|---------------------|----------|------|------|
| 进任务/专注、顺序队列、返回来源页 | `0x10 EnterTaskIn`、`0x11/0x12` 动作、`0x14 FocusStatus` | 无新 wire 字段 | **仅改固件行为** | 复用已有事件；发 `0x10` 后立即本地进页，不等 App `0x11` |
| 离线动作回放与业务确认 | `0x20/0x21`、严格 v1 `0x11/0x12`、`0x1B` | 空批、Since 与未确认动作、16B 头、版本持久化语义 | **补现有协议语义** | 命令号和主要 payload 已够用 |
| 上报 `0x1B` 快照版本库存 | Build 640 已有 `0x30` 上线通知语义；当前 v2.17 handoff 已形成 42B 电量/版本/头像/任务库库存前缀 | SnapshotState/Epoch/Revision 9B | **扩展现有命令** | 语义仍是上线库存；保留现有 42B 前缀，用 42B/51B 长度分支兼容 |
| 设备本地完整任务库 | DayPack/`0x1B` 只有概览任务和动作后快照 | 详情、三阶段文案、原子提交、独立 CRC/版本 | **新增 `0x23`** | 640 已有帧装不下离线进任务所需信息；塞进 `0x1B` 会改变其确认语义 |
| 设备本地当天内容 | DayPack 有日期、变长日程数组和当前展示文案；当前 App 最多发送 8 条日程 | 独立版本/ACK、早/闲/晚与逐事件文案、原子提交和跨日失效 | **只把日程提到 20 条时补 `0x10` 规则；要完整内容事务时新增 `0x24`** | `0x10` 的 1B EventCount 足够承载 20 条，但没有独立内容库的提交边界 |
...
1. **每帧 `0x20 EventLogRequest` 必须恰好回一帧完整 `0x21`。** 无记录时 payload 必须正好是 `Data([0x00])`；整份待回放集合的 **MVP 建议值（待固件容量签收）**为最多 32 条、重组后 payload 最多 4096B。这一帧处理完即表示本轮回放完成，**没有续页或额外终止帧**。未收到匹配 `0x1B` 的离线 `0x11/0x12` 忽略 `Since`、按原始顺序优先返回且永不被普通事件挤掉；普通事件放不下时按 best-effort 丢弃。App 在整批处理完前不下发显示数据，15 秒超时主动断开。（§4.13 / §5.15 / §5.23）
...
| **v2.18.0** | 2026-08-06 | **0x1B 快照版本双保险对齐（NVS 持久化 + 0x30 inventory 握手）。** 修复设备重启后 `0x1B TaskListSnapshotAck` 版本号从 RAM 丢失、App 与设备版本永久错位的根因。**设备侧**新增 **NVS 持久化 12B**（Epoch 4B BE + Revision 4B BE + CRC32 4B BE），CRC-32/IEEE 只覆盖前 8B；**协议侧** `0x30 DeviceWake` payload 从 **42B 扩展至 51B**，末尾新增 `SnapshotState(1B)` + `SnapshotEpoch(4B BE)` + `SnapshotRevision(4B BE)` = 9B 库存字段。`SnapshotState=0x00` 时三字段全为 0，App 清除该设备基线并以**新随机非零 epoch + revision 1** 恢复；`SnapshotState=0x01` 时 Epoch/Revision 非零，App 以设备 NVS 为准。NVS 长度、CRC 或非零约束任一不合法时固件必须当作 missing，不得上报损坏版本。**此机制与任务库库存（§4.22）完全独立**。快照 missing 属优先恢复，不受 DeviceWake 10 秒节流阻挡。§4.21/§5.8 |
...
**使用场景：** DayPack (0x10)、TaskLibraryTransaction (0x23)、DailyContentTransaction (0x24)、CustomAvatarFrame (0x15) 及其他大 payload 使用此格式。已废止的 TaskInPage (0x11) 历史帧也曾使用该格式。简单命令（PetStatus、Weather、Time）使用 Section 3.1 的 3 字节头部。
...
| `0x11` | TaskInPage（已废止） | 历史任务详情页数据；新 App 不发送，新固件不等待 |
...
设备「概览」帧的完整每日数据包（对应 §6 的常驻框架 + 数据面板）。专注态（态 C）**不在此包内**：详情与三阶段文案来自设备本地已提交的 `0x23` 任务库，实时专注数据来自 `0x14 FocusStatus`。DayPack 是丰富内容全量包，不是完成/跳过的业务确认；任务动作后的即时权威清单见 §4.21 `TaskListSnapshotAck(0x1B)`。
...
**设备行为：**
- 首先选出所有尚未收到匹配 `0x1B` 的 `CompleteTask(0x11)` / `SkipTask(0x12)`；这些记录**忽略 `Since`**，按原始发生顺序排在最前，避免水位已推进但业务确认丢失时永久漏回放。
- 其后再取本地环形缓冲中 `timestamp > Since` 的其他事件，同样保留原始线序，不按 RTC 重排。
- **整份待回放集合**（未确认任务动作 + 可选普通事件）的 **MVP 建议值**是最多 32 条；一帧完整 `0x21` payload（`Count + Records`）建议不超过 4096B，可用 §3.2 的 11B 分片头承载。这两个数是待固件容量签收的参数，不是 Build 640 已有合同；固件若不支持，必须在开工前回报实测值，双方冻结一次共享常量后再联调。固件不得拆分单条 Record。
- 未确认的 `0x11/0x12` 是强一致记录，**不得淘汰**。其他事件是 best-effort：普通事件入队会超过记录数或字节上限时，不入持久队列并记录 `event_log_best_effort_drop`；新任务动作入队前可先从最旧普通事件开始释放空间。只有清完普通事件仍放不下任务动作时，才按下条拒绝本次任务动作。这样任何时刻的全部待回放记录都能装进唯一一帧逻辑 `0x21`。
- 每帧 `0x20` **恰好回一帧**完整 `0x21`。有记录时，App 完整重组、解析和处理这一帧后，本轮回放即完成；**不存在续页、连接内页游标或额外终止帧**。
- 无记录时必须回 payload 正好为 `Data([0x00])` 的空 `0x21`（`Count=0`、无 Records、无尾字节），该帧本身即表示本轮完成。
- 强一致任务 outbox 已达到双方签收上限，或清完普通事件后加入新任务动作仍会超过签收字节上限时，固件不得覆盖旧任务动作，也不得在没有持久记录的情况下先改本地任务状态。本次新离线任务动作必须拒绝，保持当前页面/任务不变，显示静态的“请先同步”提示，并在串口记录 `event_outbox_full`；重连完成已有回放并收到对应 `0x1B` 后才释放槽位。

> **⚠️ v2.16.0：本帧已升级为每个连接的强制前置握手。** App 在安全握手完成后立即发本帧，在唯一对应 `0x21` 整批处理完之前不发送任何 presentation 数据；15 秒未完成即判连接失败并主动断开。完整契约见 **§5.15 / §5.23**。
...
**固定头是 16B**：从 `SubVersion` 到 `TaskCount` 恰好占 offset `0...15`，`Tasks[]` 从 offset `16` 开始。即使 `TaskCount=0`，payload 也仍须正好有 16B；把 `TaskCount` 误算到变长区会让空快照被错误截断。
...
2. `StateEpoch` 变化时只接受新世代的 `Revision=1`；同一 epoch 只接受 `Revision` **严格大于**当前已应用 revision 的新快照。完全相同的 epoch/revision 是传输重试：保持已应用状态、不重写 flash；更旧版本丢弃。
...
8. `DayPack(0x10)` 与 `TaskListSnapshotAck(0x1B)` 以**完整消息**为单位串行发送：一条消息的简单帧或全部分片写完后，下一条任务状态消息才能开始。固件同样必须在完整重组并校验后再应用，不能按单个分片改任务清单。

**App 生成规则：** 新建快照世代时，`StateEpoch` 必须用安全随机源生成 **UInt32 非零值**，不得固定写 `1`；该世代首帧 `Revision` 固定为 `1`，后续同 epoch 严格递增。`StateEpoch + Revision` 保存在同一个原子替换文件中，不能拆成两次 `UserDefaults` 写入。App 在任务状态消息闸内先持久化新版本，再读取当时最新的 Overview 清单并冻结完整 payload；`0x1B` 的全部写入及同一 payload 的 GATT 重试结束后才释放闸，保证 DayPack 不会插入其中。版本持久化失败时不发送半成品确认。
...
5. 设备进入已提交任务时直接读取本地记录，并按本地专注分钟选句，不等待 App 下发现场 `TaskInPage(0x11)`，也不请求现场 AI。旧 `EnterTaskIn → TaskInPage` 仅作为迁移期历史保留。
...
**新固件短按边界（v2.18.0，wire 不变）：**
- 设备无论 BLE 是否连接，都立即按本地已提交的 `0x23` 任务库进入队首任务；有连接时先发 `EnterTaskIn` 通知 App 建立专注会话，但不等待 App 返回任何详情帧。
- 本地任务库缺失或 TaskId 无效时显示空态并回到 Interactive/Overview；库缺失按 §5.8 上报 missing，由 App 补发完整 `0x23`，不得恢复现场 `TaskInPage` 请求。
- Complete/Skip 的最终业务状态仍由匹配的 `0x1B` 确认。A/C 迁移阶段曾使用“最多等待 5 秒后本地兜底”，仅作 §4.22 历史，不是 v2.18.0 新固件要求。
...
| 42     | SnapshotState | 1 byte | **v2.18.0：** `0x00`=快照版本缺失，`0x01`=有已应用版本 |
| 43     | SnapshotEpoch | 4 bytes | Big Endian；缺失时填 0，已提交时必须非零（来自 NVS 持久化） |
| 47     | SnapshotRevision | 4 bytes | Big Endian；缺失时填 0，已提交时必须非零 |

> **长度分支：** 对齐 BLE v2.18.0 / 固件规格 v1.14.0 的实时 DeviceWake payload 固定 **51B**；上一个库存格式为 42B，更旧格式为 29B 等。App 仍能读取旧 0/1/4/9/29/42B 唤醒包，但只有 51B 包能报告 `0x1B` 快照版本库存。不得在 42B 分支中读取 Snapshot 字段。
>
> **MVP 边界：** 51B 库存只覆盖头像、`0x23` 任务库和 `0x1B` 快照版本，**不含 `0x24` 当天内容库存**。设备正常重启必须从自己的 committed 区恢复 `0x24`；若产品还要求当天内容分区丢失后由 App 自动补回，MVP 优先让 App 每次重连幂等重发当天 `0x24`，不继续扩展 wire。只有以后确实要减少重传流量时，才考虑扩展实时 `0x30` 上报 `0x24` 库存。不能把 51B Wake 写成已经覆盖这项恢复。
>
...
>
> **NVS 持久化要求（BLE v2.18.0 / 固件规格 v1.14.0）：** 设备须在成功原子应用 `0x1B` 快照时，将版本记录以固定 12B 一次写入 NVS：`SnapshotEpoch(4B BE) | SnapshotRevision(4B BE) | CRC32(4B BE)`。CRC 算法固定为 **CRC-32/IEEE (ISO-HDLC)**：`poly=0x04C11DB7`、`init=0xFFFFFFFF`、`refin=true`、`refout=true`、`xorout=0xFFFFFFFF`；只覆盖前 8 个 wire-order 字节（即两个 Big Endian 整数），不包含 CRC 字段自身，结果以 Big Endian 存放。
>
> 启动读取时，长度不是 12B、NVS 读取失败、CRC 不匹配、Epoch=0 或 Revision=0 任一成立，都必须把该记录当作**缺失/损坏**：不得用其接受/拒绝新快照，不得把其中的值上报给 App；可删除该坏记录。`DeviceWake` 必须发 `SnapshotState=0x00` 且 Epoch/Revision 全 0。只有长度、CRC 和非零约束全部通过时才发 `SnapshotState=0x01`。
...
5. **若 payload = 51B（v2.18.0 双保险对齐）：** `SnapshotState=0x00` 时三个版本字段必须全为 0，App 清除该设备的 `0x1B` 版本基线并触发立即刷新；恢复时生成**新随机 UInt32 非零 epoch**，其首帧 `revision=1`，不得固定使用 `epoch=1`。`SnapshotState=0x01` 时 Epoch/Revision 必须非零，App 采纳设备上报版本作为新基线（若 App 本地记录与设备不同，以设备为准——设备 NVS 是权威来源）。其他组合视为无效库存，不改变已知状态。此机制与任务库库存独立，互不影响。
...
**有界单批与完成（v2.18.0 契约订正）：**

- **施工默认值（MVP 建议、待固件容量签收）**为整份待回放集合 `Count <= 32`，且重组后的完整 `0x21` payload `<= 4096B`。这不是 Build 640 历史合同；若实测 Flash/RAM 不支持，固件须在开工前给出实测容量，双方只改一次共享常量再冻结。超出当前 MTU 时允许按 §3.2 的 11B 分片头承载，但分片只是传输层，App 完整重组后才解析该批次。
- 每帧 `0x20` 恰好对应一帧完整 `0x21`。非空帧的全部 Records 处理和任务动作 `0x1B` 确认完成后，本轮即完成；不再发续页请求。
- 无记录时 payload 必须正好是 `Data([0x00])`（`Count=0`、无 Records、无尾字节）。该空帧本身即是完成帧；非空批次后不得再发额外空帧。
- 存储和入队规则见 §4.13：未确认 `0x11/0x12` 永不淘汰；普通事件放不下时 best-effort 丢弃，新任务动作可先淘汰最旧普通事件。若清完普通事件仍达到双方签收上限，按 §4.13 拒绝新任务动作。因此一帧可完整返回当时的**全部**待回放记录。
...
**任务动作回放：** 固件对未收到匹配 `0x1B` 的 `0x11/0x12` **忽略 `EventLogRequest.Since` 并优先放入回放流**，然后才继续普通的 `timestamp > Since` 记录。`OperationID` 是幂等主键；完全相同的重放返回**首次处理时缓存的 Result** + 最新 `0x1B`，同 ID 不同 payload 返回 `invalidRequest`，不得再次翻转任务或结束后来的专注会话。App 必须按单批 Records **线序**逐条处理、逐条返回 `0x1B`，不得按 Timestamp/RTC 重排。固件需为离线队列中每个 Action+OperationID 保留待确认记录，逐帧匹配清除。
...
- durable task outbox 与普通事件共用 §4.13 经双方签收后的整批上限（施工默认建议为 32 条/4096B），但任务动作优先且永不淘汰。新任务动作先清最旧普通事件；若仍无法持久化，本次动作是上述“立即本地执行”的唯一例外：不改任务库/队列，显示“请先同步”，并记录 `event_outbox_full`。绝不得丢旧任务动作换新记录。
...
| 等待 | 等待唯一对应的完整 `0x21`；非空批次完成**解析 + 顺序状态变更 + 逐条 `0x1B` 确认**后即完成，空批次则必须精确为 `Data([0x00])` |
| 超时 | **15 秒** |
| 失败（写入失败 / 超时 / 批次解析失败） | App **判定本次连接失败并主动断开**，不进入正常同步 |
| 成功 | 才允许生成和写入任何 presentation 数据（DayPack `0x10`、任务库事务 `0x23`、当天内容包 `0x24`、`0x14` 等） |

> ⚠️ **固件必须对每一帧 `0x20 EventLogRequest` 恰好回一帧完整 `0x21 EventLogBatch`。** 无记录时回精确 `Data([0x00])`；有记录时回一帧有界非空批次，处理完即结束，不追加空帧。不回会让 App 在 15 秒后判连接失败并主动断开。

- **回放完成之前，App 不把这次连接算作成功。** 窗口位于连接流程内部——安全握手通过后立即开窗，回放成功才向上层上报「已连接」。因此固件不回 `0x21` 时，App 侧观察到的是**连接失败**（而非「连上了但没数据」），随后走正常重连退避，循环往复。
- **每个连接只开一个回放窗口、发一帧强制 `0x20`。** 连接期内并发的其他同步触发共用这次等待，不另开回放流。
...
① **无离线事件**时收到 `0x20 EventLogRequest` → 回精确空批次 `Data([0x00])`，App 正常进入同步（**最易漏**）；
② 强一致任务 outbox 达到双方签收上限时再做一次离线完成/跳过 → 新动作不生效、旧记录不被覆盖，页面显示“请先同步”且串口记录 `event_outbox_full`；
③ 断连期间完成一次、跳过一次 → 重连后按**实际发生顺序**放在同一帧 `0x21` 内回放，App 逐条回 `0x1B`；即使两条时间戳都 `<= Since`，只要未确认仍须优先回放；
④ 收到 `0x1B` 前断电重启 → 重连后同一 OperationID 与 payload **原样重发**，App 返回首次结果，设备与 App 状态都不再二次变化；
⑤ 故意不回对应 `0x21` → App 在 15 秒后主动断开（用于确认 fail-closed 行为，非正常路径）。
...
**数据来源：** 设备本地已提交的 `0x23` 任务库（标题、描述、三阶段文案）+ `0x14 FocusStatus`（专注状态与能量瓶实时推送）。设备仍发送 `0x10 EnterTaskIn` 通知 App 建立专注会话，但 App 不回发 `0x11`。**不在 DayPack 内。**
...
2. **右侧面板改由结构化字段驱动**：
   ...
   - 专注详情（态 C）：改从设备本地已提交的 `0x23` 任务库读取；`0x14 FocusStatus` 继续提供实时专注数据。
...
| 专注任务详情 | 已提交 TaskLibraryTransaction(0x23) 本地记录 | v2.18.0 当前路径 |
| 能量瓶 | SettlementData.totalEnergyBottles | 已有 |

**已定决策（v2.5.0）**

- **气泡语气**：直接用 App 的 `currentPetDialogue`（阶段感知），接受其按时段变脸的语气。不在硬件侧另取一句。
- **面板态**：DayPack 只承载「概览」数据（PetDialogue + Events[] + TopTasks[] + 进度/能量来自 SettlementData）；**不引入 `PanelMode` 字节**，设备按现有数据渲染。专注态（态 C）由设备本地已提交的 `0x23` 任务库 + `0x14 FocusStatus` 独立驱动；`0x10 EnterTaskIn` 只通知 App 建立专注会话，不在 DayPack 内。⚠️ 设备端「进度条 vs 任务清单」二选一的布局规则需固件/产品在重写时定，App 两者都发。
...
| 优先级 | 条件（固件本地判定） | 渲染 |
|---|---|---|
| 1 | 设备已从本地 committed `0x23` 任务库进入任务且会话尚未结束（结束/中断判定见 §9） | 态 C（专注详情 + 能量瓶） |
...
| 新固件任务短按触发 `EnterTaskIn` | 立即从本地 committed `0x23` 任务库进入专注页；不建立 `TaskInPage` 等待态或计时器 |
...
| EnterTaskIn | 0 秒 | 发出通知后立即使用本地 committed `0x23` 任务库；库缺失时显示空态并上报 missing，不等待 `TaskInPage` |
...
1. **DayPack 偏移 3 的 DeviceMode 不是专注态信号。** 它是设置类快照字段，App **当前实现恒发 `0x00`**（Interactive，进不进专注都不变；`0x01=Focus` 仍是合法 wire 值、保留不废），且专注态本就「不在此包内」（§4.7 开头原文，当前由本地 committed `0x23` 任务上下文 + `0x14` 驱动）。据它判定「是否在专注」永远得到「否」，心跳被恒假信号关死。
2. **§5.7 周期唤醒的生命周期只绑设备本地会话上下文与连接**：**进入专注页 → 启动**（A 阶段曾由收到 `0x11 TaskInPage` 建立任务上下文；**B/v2.18.0 新固件**改由设备本地从已提交任务库进入队首任务时建立——仍绑 `0x11` 会导致心跳永不启动、息屏期间能量瓶停更，见 §4.22 与 §5.7 的迁移注）；发出 CompleteTask/SkipTask、**连接断开、本地状态丢失或设备重启** → 停止。专注进行中的实时进度权威是 `0x14 FocusStatus`（Phase `1=warmup/2=building/3=deep` 为活跃、`0=idle`，按当前未打断段计），态 C 的建立与退出由本地 committed `0x23` 任务上下文 + 本地会话事件驱动——**不得**用 DayPack 的任何字段改变专注页状态或心跳。
