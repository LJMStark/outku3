...
| v2.6.1 | 2026-07-23 | **CustomAvatarFrame(`0x15`) 新增 SubVersion `0x03`（KRI 载荷，固件待实现；当前 wire 默认不变）**：§4.12 定义 v3 payload = `SubVersion(0x03) \| KRI v1 文件字节`（≤2,240,012B；KRI = 12B 小端文件头 + 左上起始、逐行、直通 alpha 的 BGRA 裸像素，完整格式与转换规范见 `docs/KRI_图片转换规范.md`）——固件收到后免 PNG 解码，校验文件头与总长即可直接交 LVGL（ARGB8888）。**App 侧已完整实现**（推送前由同一张 wire PNG 现场转换 + KRI 规范 §7 严格校验 + 重试/换机重推/CRC 对账全套机制复用），由硬件调试开关「Avatar KRI Push」控制，**默认 OFF = 仍发 0x02 PNG**；固件实现 0x03 并双方同机 A/B 验收后再 flag-day 切默认。固件在实现前维持「SubVersion ∉ 已实现集合 → 丢弃整帧」即可，**无兼容风险**。§3.2 传输预估补 KRI 最坏情况 ≈4,472 片 / 约 4 分钟；§5.8 `AvatarCRC32` 口径明确为「最近一次成功接收并持久化的 0x15 文件字节」（PNG 或 KRI，格式无关）。§4.1/§4.12/§3.2/§5.8 |
...
| `0x15` | CustomAvatarFrame | 推送用户自定义伴侣头像（≤800×700、保持原图比例）。默认 SubVersion `0x02` = **PNG**（尽力 ≤1MiB，v2.5.24）；v2.6.1 另定义 SubVersion `0x03` = **KRI 裸像素**（≤2,240,012B，固件待实现，App 默认不发），详见 §4.12 |
...
App→Device 推送用户自定义伴侣的头像：用户在 App 内选用自定义伴侣形象时，将其照片处理后推送到 E-ink 显示。**v2.5.24 起按硬件团队要求（2026-07）改传 PNG 文件**（SubVersion `0x02`，当前默认）：App 侧不再做 Spectra-6 量化与 4bpp 打包，**色彩量化 / 抖动由固件在渲染时完成**；App 只保证 PNG 格式、尺寸与字节预算（见下）。App 端已实现并在运行（用户选择/创建自定义伴侣、以及 BLE 重连补推时发送）。**v2.6.1 另定义 KRI 裸像素载荷**（SubVersion `0x03`，见下方 v3 结构）：免固件 PNG 解码，校验后直接交 LVGL——固件实现并双方验收后将 flag-day 切为默认。
...
| Offset | Field      | Size            | 描述 |
|--------|------------|-----------------|------|
| 0      | SubVersion | 1 byte          | 格式版本，当前默认 `0x02`（PNG 载荷）。`0x01`（4bpp 96×96）已废弃，固件**无需实现**；`0x03`（KRI 载荷，v2.6.1）见下方 v3 结构；**SubVersion 不在已实现集合内时应丢弃整帧**（实现 0x03 前即「≠0x02 丢弃」，行为不变） |
...
**重组后 Payload 结构（v3，SubVersion 0x03 —— KRI 载荷，v2.6.1 新增，固件待实现）：**

| Offset | Field      | Size            | 描述 |
|--------|------------|-----------------|------|
| 0      | SubVersion | 1 byte          | 固定 `0x03`（KRI 载荷） |
...
**App 侧对 KRI 的保证（与 PNG 同源同约束）：**
- 由同一张 wire PNG（上方 v2 的产物，≤800×700 保比例）推送前现场转换（`KRIEncoder`），尺寸与比例约束与 PNG 完全一致；
...
2. **持久化**收到的 KRI 文件（建议路径 `/assets/characters/custom/avatar.kri`）；自定义头像资产同一时刻只存一份——收到新 0x15 帧（无论 0x02 还是 0x03）即覆盖旧资产；
3. §5.8 DeviceWake 的 `AvatarState`/`AvatarCRC32` 口径**格式无关**：对**最近一次成功接收并持久化的 0x15 文件字节**（PNG 或 KRI）计算 CRC-32/IEEE，存图时算一次并缓存（勿每次唤醒重算 2.2MB）；
4. 渲染：KRI 直接作为 LVGL ARGB8888 图片参与页面合成（渲染语义与 §4.2 `CustomActive` 的既有约定完全一致，仅省去 PNG 解码一步）；
5. **实现 0x03 前无需任何改动**：维持「SubVersion ≠ `0x02` 丢弃整帧」即可——App 默认不发 0x03，无兼容风险。

**切换节奏（flag-day 同款流程）：** App 已完整实现 0x03 全链路（现场转换 + §7 校验 + 发送 + 重试 / 换机重推 / CRC 对账全部复用 PNG 机制），由硬件调试开关「Avatar KRI Push」控制（默认 OFF = PNG）。固件实现后先开开关做**同机 A/B 联调**（同一张图两种格式对拍显示效果与传输稳定性），双方验收通过后 App 将默认切为 KRI、`0x02` 转入废弃流程（届时另行修订本节）。
...
> **v2.6.0 显示口径（客户 2026-07-20 答复 + CustomActive 字节落地）**：自定义 IP 激活与否由 `0x01 PetStatus.CustomActive` 字节（§4.2，v2.6.0）**显式判定**——`=1` 时除专注页外的全部页面（早安/概览/每日总结/屏保等）显示本帧下发的用户单张图，**不随任务/标签切换**（当前仅支持单图上传）；专注页维持内置 IP 美术（按 `CharacterId`，多姿态图未支持、2026-07-14 搁置延续）；`=0`（用户切回内置）即恢复按 CharacterId 渲染。固件须**持久化**本帧载荷（PNG，v2.6.1 起 KRI 联调开关开启时为 KRI；重启不丢；App 不重复推送已成功的图）。**v2.6.0 换设备重推**：App 记录本帧成功送达的设备 id，连接到不同设备且自定义激活时自动重推本帧——固件侧无需配合；设备侧"存储被清空/图损坏/图过期"由 §5.8 DeviceWake 的 `AvatarState`+`AvatarCRC32` 上报闭环（v2.6.0 定稿）。
