...
**版本:** v2.16.0
**更新日期:** 2026-08-04
**状态:** **本版是一次合并发布**——把 2026-08-03 当天的全部内部迭代（v2.11.0–v2.15.0）并入 v2.16.0 一次交付。那些编号未曾对固件单独发布，**请不要逐版对照**；固件上一个收到的版本是 **v2.10.2**，按 v2.10.2 → v2.16.0 的整体差异施工即可。

**2026-08-04 修订（并入本版，字节仍不变）：任务库收窄为「仅当天」。** 客户拍板：`0x23` 只发送设备本地**当天**的任务——dueDate 在当天，或用户在 App 里手动设为今天的无日期任务；**未来、过期、未选入的无日期任务不再发送**（此前「全部未完成任务都发」的条款废除）。条数仍不设上限。跨自然日后 App 立即按新日重算重发（不等 180 秒稳定窗），设备旧库在新版到达前继续可用。详见 §4.22。**固件解析器无需改动**——变的是 App 发送的内容集合与验收预期，不是字节。

**本版不改任何已有帧的字节布局。** 新增两个双向帧（`0x23` 设备任务库、`0x24` 当天内容包），其余全部是**设备行为**变化：断连不结束专注、离线动作先本地生效并幂等回放、顺序任务队列与来源页面返回、墨水屏静态刷新、断电恢复与冲突合并规则。完整清单见 §1.3 修订历史的 v2.16.0 一行，固件视角的一页待办见固件功能规格文档「实施总清单」。

**四条最容易踩的，请先看：**

0. **任务库只含当天任务**（08-04 修订）——测试时放未来/无日期任务进 App 后设备收不到**不是 bug**；无日期任务要上设备，需在 App 里手动设为今天。跨日后旧任务由 App 增量 delete，设备不自行按日期过滤。
...
| `0x23` | TaskLibraryTransaction | 设备任务库事务；App 发送**当天**未完成且非待删除任务（含手动设为今天）及逐任务三阶段文案，详见 §4.22 |
...
- **显示条数与保存条数分离**：屏幕一次显示多少项，与设备实际保存多少任务无关。**不得**因页面布局反过来截断任务库，也不得设置八项或其他任意业务上限（容量不足的正确做法是整次事务失败并报 `capacityExceeded`，见 §5.21）。「不设上限」的作用域是**当天这一组**——任务库自 08-04 修订起只含当天任务（§4.22），当天有多少发多少。
