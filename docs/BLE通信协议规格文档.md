...
**版本:** v2.6.1
**更新日期:** 2026-07-23
**状态:** 电子墨水屏五页对齐**定稿**（2026-07-20，v2.6.0——同日内部迭代 v2.5.30–v2.5.33 未曾对固件发布，已合并为本版一次性发布；App 代码注释中的 v2.5.3x 标号与 v2.6.0 同义）。固件实施入口：**《固件功能规格文档》v1.6.0 开头的「实施总清单」**（一页看完要做什么）；字节级细节按本文档 **§4.2**（PetStatus 含 CustomActive）、**§4.7**（DayPack：Events[] 含 Category/EndTime，尾部 DaySummary/FirstUp/SettlementReview/SettlementQuote）、**§5.8**（DeviceWake 含 AvatarState/AvatarCRC32）、**§7.1**（当前字段顺序 + 往返自检）、**§6**（页面数据结构与流转）。v2.5.0–v2.5.29 及更早的演进历史全部在 **§1.3 修订历史**，不再在此堆叠。
...
**大帧传输预估与 MTU 建议（v2.5.24）：** 以最坏情况 ≤1MiB 头像帧（§4.12，payload ≤1,048,577B）为例：协商 512B 写长度时每片 payload = 512−11 = 501B → **≈2093 片**；叠加 App 侧写限流（§8.5，20 次/秒）**理论 ≈105 秒、实际预期 1–2 分钟**——固件看到 0x15 分包流持续约 2 分钟属正常，不是卡死。**KRI 载荷（v2.6.1，§4.12 SubVersion `0x03`）最坏情况 payload = 2,240,013B → ≈4,472 片、理论 ≈224 秒、实际预期 4–5 分钟**，同样属正常传输而非卡死；仍远低于 65,535 片上限。**强烈建议固件协商大 MTU**（写长度 ≥512）；写长度过小会使片数超过 65535，此时 App 侧组包直接报错、该帧转入 §4.12 的待重发队列，**不会发出半截消息**。
...
| `0x15` | CustomAvatarFrame | 推送用户自定义伴侣头像（≤800×700、保持原图比例）。默认 SubVersion `0x03` = **KRI 裸像素**（≤2,240,012B，v2.6.1）；SubVersion `0x02` = **PNG**（尽力 ≤1MiB，v2.5.24，联调回退通道），详见 §4.12 |
...
App→Device 推送用户自定义伴侣的头像：用户在 App 内选用自定义伴侣形象时，将其照片处理后推送到 E-ink 显示。**v2.6.1 起默认传 KRI 裸像素**（SubVersion `0x03`，见下方 v3 结构）：固件免 PNG 解码，校验文件头与总长后直接交 LVGL；**色彩量化 / 抖动仍由固件在渲染时完成**（与 v2.5.24 约定一致）。旧 **PNG 载荷**（SubVersion `0x02`，v2.5.24 引入）保留为**联调回退通道**——App 调试开关切 OFF 时发送，供固件同机 A/B 对拍；该 PNG 同时是 App 侧 KRI 转换的源，故两种载荷的尺寸约束一致。App 端已实现并在运行（用户选择/创建自定义伴侣、以及 BLE 重连补推时发送）。
...
**传输方式：** 经 §3.2 分包帧发送（**11 字节分包头**，`type=0x15`）；固件按分包重组后得到下方完整 payload。**不以简单包形式出现。** 默认 KRI 最坏为 2,240,013B payload，约 4,472 片 @501B/片、传输 4–5 分钟；回退 PNG 最坏为 1,048,577B、约 2,093 片、传输 1–2 分钟（详见 §3.2 大帧传输预估）。期间其他业务帧可能以不同 MessageId 交错到达。

**重组后 Payload 结构（v3，SubVersion 0x03 —— KRI 载荷，v2.6.1 起默认）：**

| Offset | Field      | Size            | 描述 |
|--------|------------|-----------------|------|
| 0      | SubVersion | 1 byte          | 固定 `0x03`（KRI 载荷，默认格式） |
| 1      | KRIFile    | N ≤ 2,240,012 B | 完整 KRI v1 文件字节：12B 文件头（Magic `4B 52 49 01` \| Width 2B **小端** \| Height 2B **小端** \| ColorFormat=`0x01`(ARGB8888) \| Version=`0x01` \| Reserved `00 00`）+ 左上起始、逐行、**直通（非预乘）alpha** 的 **BGRA** 裸像素。总长恒为 `12 + Width × Height × 4`，无压缩、无行 padding、无尾部数据。完整格式与转换规范见 `docs/KRI_图片转换规范.md` |

总 payload 长度 ≤ `1 + 2,240,012 = 2,240,013` 字节（尺寸封顶 800×700 即字节封顶；≈4,472 片 @501B/片，传输约 4–5 分钟，见 §3.2）。

**动因：** KRI 为无压缩裸像素，固件收到后**免 PNG 解码**——按 KRI 规范 §7 校验文件头与总长即可直接作为 LVGL `ARGB8888` 图片合成（`LV_COLOR_FORMAT_ARGB8888`，直通 alpha，非预乘变体）；六色量化 / 抖动照旧在刷新阶段完成。

**App 侧对 KRI 的保证：**
- 由 wire PNG（下方 v2 的产物，≤800×700 保比例）推送前现场转换（`KRIEncoder`），尺寸与比例约束与 PNG 完全一致；
- 发送前按 KRI 规范 §7 严格校验（魔数 / 宽高非零 / colorFormat / version / 保留位 / 总长逐字节相等），不合法**不发出**（弃推并记日志，与 4bpp 旧资产护栏同策略）；
- alpha 恒为直通（非预乘）；EXIF 方向已烘焙进像素；不会出现行 padding 或尾部附加数据。

**固件实现要点：**
1. `SubVersion=0x03` → 按 KRI 规范 §7 校验：Magic、宽高 >0、`ColorFormat=0x01`、`Version=0x01`、保留位为零、**总长严格等于 `12 + W×H×4`**（多一字节少一字节都丢弃整帧）；
2. **持久化**收到的 KRI 文件（建议路径 `/assets/characters/custom/avatar.kri`）；自定义头像资产同一时刻只存一份——收到新 0x15 帧（无论 `0x03` 还是回退期的 `0x02`）即覆盖旧资产；
3. §5.8 DeviceWake 的 `AvatarState`/`AvatarCRC32` 口径**格式无关**：对**最近一次成功接收并持久化的 0x15 文件字节**（KRI 或 PNG）计算 CRC-32/IEEE，存图时算一次并缓存（勿每次唤醒重算 2.2MB）；
4. 渲染：KRI 直接作为 LVGL ARGB8888 图片参与页面合成（渲染语义与 §4.2 `CustomActive` 的既有约定完全一致，仅省去 PNG 解码一步）；
5. **实现前的空窗行为**：未实现 `0x03` 期间按既有规则丢弃未知 SubVersion 整帧即可。App 不把 GATT 分片写回执当作固件持久化成功：`0x03` 会保持待确认，并按下方退避策略重发；收到 §5.8 匹配的 `AvatarCRC32` 后才停止。旧固件没有头像库存上报时也会继续低频重发，不会误记为已送达。

**联调开关（App 侧）：** App 默认发 `0x03` KRI；硬件调试开关「Avatar KRI Push」切 OFF 可回退发 `0x02` PNG，供固件开发期**同机 A/B 对拍**（同一张图两种格式对比显示效果与传输稳定性）。`0x02` 的最终废弃时机待双方验收后另行修订本节。

**重组后 Payload 结构（v2，SubVersion 0x02 —— PNG 载荷，联调回退通道）：**

| Offset | Field      | Size            | 描述 |
|--------|------------|-----------------|------|
| 0      | SubVersion | 1 byte          | 固定 `0x02`（PNG 载荷，联调回退；默认格式为上方 v3 的 `0x03` KRI）。`0x01`（4bpp 96×96）已废弃，固件**无需实现**。**SubVersion 不在已实现集合内时应丢弃整帧** |
...
**真相源：** 出站字节见 `Core/BLE/BLEProtocol.swift` 的 `BLEDataType.customAvatarFrame`；编码见 `BLEDataEncoder.encodeCustomAvatarFrame`；图片处理见 `AvatarImageProcessor`（800×700 / 1MiB / KRI 2,240,012B / ×0.9 收缩常量均在此）；KRI 转换见 `KRIEncoder`。

**App 侧重发策略（联调相关，v2.6.1）：** 自定义头像帧由 App 在切换伴侣或 BLE 重连后尝试推送。传输失败会进入待重发；默认 `0x03` 即使 GATT 写完也保持待确认，直到 DeviceWake CRC 与本地 KRI 一致。后续每轮 sync 按**退避降频**重试，而不是使用固定次数硬上限：
...
- `0x03` 收到匹配 CRC、`0x02` 回退传输成功、或用户切换到其它伴侣后，重试计数清零。

> 联调提示：固件未实现 `0x03` 或尚未回报匹配 CRC 时，会看到 App 偶发重发 `0x15`——这是预期内的低频自愈重试；固件持久化后应在 DeviceWake 回报匹配 CRC，重发随即停止。真相源：`AppState+CustomCompanions.swift` 的 `reconcileCustomAvatarInventory` / `shouldAttemptCustomAvatarFlush` / `flushPendingCustomCompanionPushIfNeeded`。
...
**⚠️ secure 模式下 0x15 暂不可用（开放问题，启用安全模式前须与固件定版）：** §3.4 的 `0x7E SecureEnvelope` 长度字段为 2 字节（≤65,535B），结构上装不下 MiB 级头像 payload（PNG ≤1,048,577B / KRI ≤2,240,013B，两种 SubVersion 同受此限）；且信封时间戳容差 120 秒，小于默认 KRI 4–5 分钟的传输时长。App 侧已防御：secure 模式下发送超过 65,535B 的信封 payload 会**抛错进入待重发队列**（不会崩溃、不会发出损坏帧），即 secure 模式启用后头像帧会持续待发直到大帧安全封装定版（候选方案：长度字段扩 4B / 分片签名 / 0x15 豁免走明文——需双方共同确认）。当前联调按明文运行，不受影响。
...
> **v2.6.0 显示口径（客户 2026-07-20 答复 + CustomActive 字节落地）**：自定义 IP 激活与否由 `0x01 PetStatus.CustomActive` 字节（§4.2，v2.6.0）**显式判定**——`=1` 时除专注页外的全部页面（早安/概览/每日总结/屏保等）显示本帧下发的用户单张图，**不随任务/标签切换**（当前仅支持单图上传）；专注页维持内置 IP 美术（按 `CharacterId`，多姿态图未支持、2026-07-14 搁置延续）；`=0`（用户切回内置）即恢复按 CharacterId 渲染。固件须**持久化**本帧载荷（默认 KRI，v2.6.1；联调回退期收到的 PNG 同待遇；重启不丢；App 不重复推送已成功的图）。**v2.6.0 换设备重推**：App 记录本帧成功送达的设备 id，连接到不同设备且自定义激活时自动重推本帧——固件侧无需配合；设备侧"存储被清空/图损坏/图过期"由 §5.8 DeviceWake 的 `AvatarState`+`AvatarCRC32` 上报闭环（v2.6.0 定稿）。
...
| 4      | AvatarState  | 1 byte | v2.6.0：`0x00`=本地**无**自定义头像图 / `0x01`=有（已持久化的 0x15 载荷；v2.6.1 起格式无关——PNG 或 KRI） |
| 5      | AvatarCRC32  | 4 bytes | v2.6.0：已持久化 0x15 文件字节的 **CRC-32/IEEE**（Big Endian；AvatarState=0 时填 0）。口径 = **最近一次成功接收并持久化的 0x15 文件字节**（PNG 或 KRI，v2.6.1 明确格式无关；App 侧按当前推送格式计算比对值）。固件**存图时计算一次并缓存**，勿每次唤醒重算 MiB 级文件 |

> **固件版本要求：** `BatteryLevel` 自 v2.3.0+ 必填；固件版本 3 字节自 v2.5.19+ 必填；**头像库存 5 字节（AvatarState+AvatarCRC32）自 v2.6.0+ 必填（整包 payload = 9B，Length = `0x09`）**。头像库存的动因（行业通行做法：设备上报所持资产的校验和，主机比对、不一致才重传）：固件持久化 0x15 图只覆盖"同一台重启"，**存储被清空/图损坏/图过期** App 侧无法感知——本字段闭环该盲区，CRC 防损坏足够（头像非安全资产，无需加密哈希）。版本语义 = 本次启动**实际运行**的固件版本：升级成功后为新版本，升级失败自动回滚后为回滚到的旧版本。App 向后兼容：收到 1B payload（旧固件）时电量正常解析、版本视为未知；空 payload（Length = 0）时忽略电量更新、保持上次已知值。示例：`30 04 64 01 02 03` = 电量 100%、固件 v1.2.3。
>
...
3. 若 payload ≥ 9 字节且自定义形象激活：比对本地激活头像的 CRC-32——设备无图或 CRC 不一致 → 重新标记 0x15 待推、随本轮 sync 补发（v2.6.0；App 向后兼容 0/1/4B payload，短 payload 跳过此对账）。
4. 同步时间并发送更新数据。
