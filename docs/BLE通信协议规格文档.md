...
5. **实现前的空窗行为**：未实现 `0x03` 期间按既有规则丢弃未知 SubVersion 整帧即可——App 侧退避重发（下方「App 侧重发策略」，永不放弃）会在固件就绪后自动补推，与「固件未实现 `0x15` 期间」的既有联调提示同款，**不是 bug**。
