...
**版本:** v2.5.4
**更新日期:** 2026-06-28
**状态:** DayPack 显示模型重写（1 气泡 + 数据面板）。**破坏性变更：固件需按新 §4.7 / §6 重写 DayPack 解析（与 §8.7 修复一并做）。** FocusStatus(`0x14`) 的 `ElapsedTime` 语义已改为「当前未打断连续段分钟」（v2.5.4），固件若用它显示总时长需对齐。
...
| v2.5.4  | 2026-06-28 | **FocusStatus(`0x14`) 字段语义修订（wire 字节不变，含义变）**：`ElapsedTime` 从「本会话累计专注分钟」改为「**当前未打断连续段**的专注分钟」——被手机打断即归零重计，用于驱动端上「装填进度条」在打断时归零；`Bottles` 改为按未打断段各自 floor(段÷30) 累计、零头不跨打断合并（满 30 分钟连续专注收 1 瓶，与 App 结算口径一致）。**固件若用 `ElapsedTime` 显示「总专注时长」会被打破，需对齐为「当前段」语义**（如固件仍需总时长，与 App 商定新增独立字段）。详见 §4.11 |
...
| 2      | ElapsedTime  | 2 bytes BE  | **当前未打断连续段**的已专注分钟数：被手机打断即归零重计，驱动端上「装填进度」；**非**本会话累计总分钟（Big Endian UInt16，clamp 0-65535） |
