...
| v2.5.33 | 2026-07-20 | **自定义 IP 生命周期补全 + 文案边界修订（纯 App 行为 + 文档，wire 字节不变；外部复审第四轮修复）**：① **删除激活中的自定义形象** = 一次身份切换，与选内置/选自定义同待遇：立即单发 0x01（CustomActive=0）+ 触发 sync，硬件即时切回内置（原缺口：最长滞后 1h/4h 节流窗仍显示已删除的用户图）；② **换设备自动重推 0x15**：App 侧记录头像成功送达的设备 id（`connectedPeripheral.identifier`），例行 sync 的 flush 检查发现"当前连接设备 ≠ 上次收图设备（含从未记录）且自定义激活"即重新标记待推——固件持久化覆盖"同一台重启"，本机制覆盖"换硬件/新设备"；**已知边界（需固件配合才可闭环）**：同一台设备存储被清空（持久化图丢失但设备身份未变）App 无法感知，候选方案为 DeviceWake(0x30) 追加"无自定义图"位，暂不实施、待固件团队评估；③ **CJK 死线标题口径**：全 CJK 标题（如"合同付款截止"）经 §3.5 ASCII 净化后为空串，概况点评不再硬插（硬件上只会剩 "On the deadline side: ."）——兜底与输出校验均按净化预览处理：有可上 wire 的标题仍必须点名其一，全部不可上 wire 时文本至少含 "deadline" 一词、兜底用通用表述；④ 离线 daySummary 兜底繁忙判定升级：相邻间隔 <60 分钟（前场 EndTime→后场 Time）或 ≥4 场 → 休息建议，否则（松散 2-3 场）提醒喝水；解析失败/含全天事件按紧凑保守处理。固件无需任何改动 |
| v2.5.32 | 2026-07-20 | **PetStatus 追加 `CustomActive` 字节 + 结算进度分母修正（破坏性，flag-day 窗口内；外部复审第三轮 P0/P1 修复）**：① `0x01 PetStatus` 在 `CharacterId` 后追加 1 字节 `CustomActive`（`0x01`=自定义形象激活 / `0x00`=内置）——修复复审 P0：此前例行 sync 的 0x01（内置 id）会按 §4.2"立即重绘"把 0x15 用户图刷回内置形象，且"切回内置"与"例行 sync"字节不可区分。语义拍板：`CharacterId` 恒为**最近一次内置选择**（供专注页美术，客户口径"专注页维持内置"）；`CustomActive=1` → 固件除专注页外显示**已持久化**的 0x15 用户图（本帧不再触发内置形象重绘）；`=0` → 按 CharacterId 渲染内置。**固件须把 0x15 PNG 持久化到本地存储**（重启/重连不丢，App 不重复推送成功过的图；用户重选自定义会触发重推）。App 侧：例行 sync 每轮携带真实状态；选自定义/选内置均立即单发一帧 0x01（v2.5.20 待遇扩展到自定义）。② `SettlementData.TasksCompleted/TasksTotal` 口径修正（复审 P1"1/1 应为 1/4"）：**分母含全部今日日程**（含未发生），分子仍只计已完成任务+已结束日程——清晨 0/N、白天真实中间态（客户 mock 的 50%）、日终 N/N；"防清晨误显满分"的原始动机在分子、不受影响。③ §8.7 问题 1 整改指引更新为现行字段序（原指引仍引用 pre-v2.5.0 旧字段名——v2.5.0 起的历史遗留，复审揪出）；§5.9 DeviceSleep 补记 App 实际会回发 0x16 屏保帧（原"无需响应"与实现相反）；§4.7 v2.5.8 注"三个尾字段"随 v2.5.31 撤字段改"两个"。口径备注：时长阈值（>2h/>4h）按**整分钟 floor** 判定（亚分钟误差 <1 分钟，产品可接受）；离线兜底 daySummary 按事件数量启发（精细时段判断在 AI 路径） |
...
| N+2    | CharacterId  | 1 + N bytes | 10 bytes   | 伴侣 IP：`joy` / `silas` / `nova`——恒为**最近一次内置选择**（自定义激活期间供专注页美术，v2.5.32） |
| ...    | CustomActive | 1 byte      | -          | v2.5.32：`0x01`=自定义形象激活（除专注页显示已持久化的 `0x15` 图），`0x00`=按 CharacterId 渲染内置。§7.1 严格解析：固件读完 CharacterId 后必须再读此字节 |
...
> **发送时机（v2.5.20/v2.5.32）**：本帧每轮常规 sync 固定发送（Time 之后、Weather 之前）；此外用户在 App 内**切换伙伴（内置或自定义）时立即单发一帧**（不等 1h/4h 同步节流窗，与自定义头像 0x15 的立即推送同待遇）。

> **发送时机补充（v2.5.33）**：**删除激活中的自定义形象**也属身份切换，App 立即单发一帧（CustomActive=0）+ 触发 sync——固件收到即切回内置形象。

> **形象渲染语义（v2.5.32，取代旧"据 CharacterId 即时重绘"单句）**：`CustomActive=0x00` → 据 `CharacterId` 即时重绘内置伴侣形象；`CustomActive=0x01` → **本帧不触发内置形象重绘**，除专注页外的全部页面继续/切换显示**已持久化的 `0x15` 用户图**（专注页美术用 `CharacterId` 对应的内置 IP）。若 `CustomActive=1` 但本地无已持久化的 0x15 图（如出厂后从未收到），暂按 CharacterId 内置渲染、待 0x15 到达后切换。**固件须持久化 0x15 图**：App 对已成功送达的图不重复推送（失败才重试、重选才重推），固件重启后靠本地持久化恢复显示。
...
| 1      | TasksTotal          | 1 byte      | -          | 总项数 = 全部任务 + **全部**今日日程（clamp 0-255；v2.5.32 修正——4 个日程过了 1 个显示 1/4 而非 1/1，进度条呈真实中间态） |
...
> **v2.5.31/v2.5.32 显示口径（客户 2026-07-20 答复 + CustomActive 字节落地）**：自定义 IP 激活与否由 `0x01 PetStatus.CustomActive` 字节（§4.2，v2.5.32）**显式判定**——`=1` 时除专注页外的全部页面（早安/概览/每日总结/屏保等）显示本帧下发的用户单张图，**不随任务/标签切换**（当前仅支持单图上传）；专注页维持内置 IP 美术（按 `CharacterId`，多姿态图未支持、2026-07-14 搁置延续）；`=0`（用户切回内置）即恢复按 CharacterId 渲染。固件须**持久化**本帧 PNG（重启不丢；App 不重复推送已成功的图）。**v2.5.33 换设备重推**：App 记录本帧成功送达的设备 id，连接到不同设备且自定义激活时自动重推本帧——固件侧无需配合；**已知边界**：同一台设备持久化图被清空 App 无法感知（候选闭环方案 DeviceWake 加"无图"位，待固件团队评估）。
...
**App 响应（v2.5.32 修正，对齐实际实现与固件规格 §3.6）：** App 收到本事件后会**回发一帧 `0x16 Screensaver`**（屏保金句，见 §4.15）。固件进入屏保前应**预留收帧窗口**（或使用上次缓存的金句再断连）——原文"无需响应"与实现相反，固件照旧断连会收不到最新金句。
...
数值区（进度比例等）继续复用 `SettlementData` 定长字段（§4.7，v2.5.32 起分母含全部今日日程）。时长阈值口径备注（v2.5.32）：「>2h 必提」「>4h 分支」均按**整分钟 floor** 判定（2h00m30s 计 120 分钟、不触发 >120），亚分钟误差窗口 <1 分钟，产品可接受。
...
→ 固件须严格按 §4.7 + §3.5 解析 Header(5B) 之后的字段：**先读 1 字节长度、再读对应字节数**的变长字符串。（⚠️ 本问题记录时还是 pre-v2.5.0 旧布局；**现行字段序以 §4.7 与 §7.1「当前字段顺序」为准**——v2.5.31 起为 PetDialogue → Events[]{含 Category/EndTime} → TopTasks[] → SettlementData → DaySummary → FirstUp → SettlementReview → SettlementQuote，勿再按本段当年提到的旧字段名实现。）
