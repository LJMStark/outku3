...
**版本:** v2.11.2
**更新日期:** 2026-08-03
**状态:** v2.11.2 的 App 已通过 `TaskLibraryTransaction(0x23)` 发送全部未完成任务，不按日期或展示条数截断；零任务同样发送完整替换事务。每项任务在事务形成前生成与当前角色匹配的 `0–5 / 6–15 / 16+` 三阶段文案，首次失败只重试一次，第二次失败或三分钟截止时按角色使用本地三阶段模板；截止后的迟到结果不形成第二个版本。App 按设备保存最后确认的版本和内容 CRC。事务传输自动重试和持久待发属 Issue #18；任务编辑后三分钟稳定窗和增量更新属 Issue #19；真实固件任务库属 Issue #26。旧 `DayPack(0x10)`、`TaskInPage(0x11)` 和 `TaskListSnapshotAck(0x1B)` 迁移期继续保留。
...
5. 任务后续新增、修改、完成和删除的稳定窗增量同步由 Issue #19 定义，不在 v2.11.2 伪装成重复全量发送。
