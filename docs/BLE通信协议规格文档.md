...
> **v2.5.31/v2.5.32 显示口径（客户 2026-07-20 答复 + CustomActive 字节落地）**：自定义 IP 激活与否由 `0x01 PetStatus.CustomActive` 字节（§4.2，v2.5.32）**显式判定**——`=1` 时除专注页外的全部页面（早安/概览/每日总结/屏保等）显示本帧下发的用户单张图，**不随任务/标签切换**（当前仅支持单图上传）；专注页维持内置 IP 美术（按 `CharacterId`，多姿态图未支持、2026-07-14 搁置延续）；`=0`（用户切回内置）即恢复按 CharacterId 渲染。固件须**持久化**本帧 PNG（重启不丢；App 不重复推送已成功的图）。
