...
**版本:** v2.13.1
**更新日期:** 2026-08-03
**状态:** v2.13.1 的 App 已新增独立当天内容包 `DailyContentTransaction(0x24)`，并按设备本地自然日执行跨日失效。设备日期变化后不得读取昨日包；新包未提交时显示新日期、空日程和固件本地默认对话，任务库及进行中的任务不受影响。App 在前台跨零点或跨日恢复时先刷新新一天日历，再立即准备并发送新包，不进入三分钟修改稳定窗；断线期间不发送，重连只补当前日版本。真实固件实现属后续硬件 Issue；旧 `DayPack(0x10)` 迁移期继续保留。
