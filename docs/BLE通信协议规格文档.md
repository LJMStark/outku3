...
**版本:** v2.15.0
**更新日期:** 2026-08-03
**状态:** v2.15.0 在 v2.14.0 之上补齐**交给硬件团队开工所缺的三项设计**（仍不改 wire 字节）：① **新旧通路迁移定案**——§4.22「迁移与切换阶段」给出 A/B/C 三阶段表，明确固件在 B 阶段**停发入站 `0x10 EnterTaskIn`**、进任务只读本地已提交任务库，任务库缺失时显示空态**不得回落**旧通路；B 是固件单方切换、可单方回退，C（Issue #29 删除 `0x11`）必须等 B 真机验收通过。注意方向双义：停用的是**入站** `0x10`，**出站 DayPack 不在本迁移范围**。② **设备侧持久化与断电恢复**——新增 §8.9，含「掉电时机 × 期望结果」可测矩阵与幂等重提交要求。③ 固件交付与验收证据要求见固件功能规格文档 §8.5（含强制脱敏）。

v2.14.0 **不改任何 wire 字节，但改设备行为**，是固件 Issue #26/#27/#28 的开工契约。四项：① **断连不再结束专注**——设备保持专注页并按本地时钟继续计时与三阶段切换，App 保留同一会话，重连不切页、不重建、不清零（§9.6）；**v2.5.25–v2.13.1 的「断连即会话结束 → 退回概览」已撤销**，§5.7 与 §8.7 问题 5 就地标注，勿按旧文实现。② **`0x20 EventLogRequest` 升级为每个连接的强制前置握手**：App 收到并处理完对应 `0x21` 之前不发任何 presentation 数据，**15 秒未完成即判连接失败并主动断开**；**没有离线事件时固件也必须回一帧空 `0x21`**（§4.13 / §5.23）。③ 顺序任务队列、来源页面与静态反馈定契约（§6.8）。④ 离线动作与 App 状态的四条冲突合并优先级定契约（§8.8）。

v2.13.1 的独立当天内容包 `DailyContentTransaction(0x24)` 与跨日失效继续生效：设备日期变化后不得读取昨日包；新包未提交时显示新日期、空日程和固件本地默认对话，任务库及进行中的任务不受影响。真实固件实现属后续硬件 Issue；旧 `DayPack(0x10)` 迁移期继续保留。
...
| v2.13.1 | 2026-08-03 | **当天内容跨自然日失效（wire 不变）**：设备本地日期与 committed `Year/Month/Day` 不同时，昨日程、每日对话、屏保和结算立即不可读；页面显示新日期、空日程和本地默认对话，独立任务库与当前专注资料继续保留。App 用日历边界定时器和持久日期标记覆盖前台跨零点、挂起后恢复、App 重启与时区变化；发现新日后清除昨日程的 180 秒冻结投影，先刷新新日历再立即准备 `0x24`，不进入稳定窗。旧日待发包因日期指纹不匹配作废；重连只补当前日完整包。§4.23 |
| v2.14.0 | 2026-08-03 | **断联专注、离线动作回放与设备本地队列（wire 不变，固件行为变；固件 Issue #26/#27/#28 的开工契约）**：① **撤销「断连即会话结束」**（v2.5.25–v2.13.1 旧约定，§5.7 与 §8.7 问题 5 已就地标注、验收用例 ② 已反转）——断连不结束专注，设备保持专注页、按本地时钟继续计时并切换三阶段文案，App 保留同一会话；重连不切页、不重建、不清零，并恢复 §5.7 周期唤醒。连接中且正在专注时 App 拒绝编辑该任务；断连后可编辑，但重连不替换正在显示的专注页快照，退出后再次进入才生效。跳过不发放能量瓶。② **`0x20 EventLogRequest` 升级为每连接强制前置握手**：App 在处理完对应 `0x21` 之前不生成也不写入任何 presentation 数据（`0x10/0x14/0x23/0x24`），**15 秒未完成即判连接失败并主动断开**；**⚠️ 无离线事件时固件也必须回一帧空 `0x21`**，否则表现为「连上就断、反复重连」且无错误码。设备离线完成/跳过须先本地生效并跨断电持久化非零 OperationID、payload 与**原始发生顺序**；重发原样复用同一 OperationID 与 payload。幂等身份=（设备+Action+OperationID），载荷比对用 **wire 原始 TaskId**，故任务行消失后重发仍能匹配首次结果。回放严格先于 App 下发；回放改变状态时整轮内容重建，旧字节不上 wire。③ **顺序任务队列与来源页面**：短按只进队首，完成移除、跳过移队尾；无滚轮/高亮/任意选择，App 是调序唯一入口；显示条数与保存条数分离，不设八项或其他任意上限；专注/总结/屏保按各自来源页面返回；墨水屏只做静态刷新（无声/无震/无动画，不约束 iPhone 端）。④ **冲突合并四条固定优先级**：完成 > 内容编辑；跳过与内容编辑并存（最新内容 + 队尾）；删除 > 跳过；先取 App 最新顺序再按离线跳过先后移队尾。断联专注的任务被删除时重连不打断，完成后时长与能量照常结算但任务不恢复。§4.13/§5.7/§5.23/§6.8/§8.7/§8.8/§9.6，ADR 0010/0013/0014/0015/0016/0019/0020/0021/0022/0023/0024/0025/0026/0027/0028 |
| v2.15.0 | 2026-08-03 | **交付硬件团队前的设计补齐（wire 不变；补 v2.14.0 遗留的三项开工缺口）**：① **§4.22 新增「迁移与切换阶段」**——A（当前双通路）/ B（固件停发**入站** `0x10 EnterTaskIn`、进任务只读本地已提交任务库，Issue #26）/ C（App 删除 `0x11 TaskInPage` 生成路径，Issue #29）三阶段表。**B 是固件单方切换、不需 App 同步发版、可单方回退**；**C 必须等 B 真机验收通过**，否则仍发入站 `0x10` 的固件会拿到空白页且无错误提示。**任务库缺失时显示空态并按 §5.8 报 missing，禁止回落旧通路**（否则「设备是否已迁移」变成运行期不确定状态，C 无法安全执行）。⚠️ 方向双义提醒：本迁移停用的是**入站** `0x10 EnterTaskIn`，**出站 `0x10 DayPack` 由 `0x24` 另行接管、不在本表范围**。`0x1B TaskListSnapshotAck` 三阶段一直生效，**不在 #29 删除范围**。② **新增 §8.9 设备侧持久化与断电恢复**——三个独立持久化对象（任务库 / 当天内容包 / 离线动作）；版本标识与内容必须一起原子落盘（禁止「版本已推进但内容是旧的」）；暂存区与 committed 区物理分离，未校验暂存断电后绝不成为当前版本；附**「掉电时机 × 期望结果」可测矩阵**，其中「提交完成、应答未发出」时 App 会用同版本整包重发，**设备须幂等回 `committed`、不重复擦写 flash**。③ 固件交付与验收证据协议（固件版本、协议版本、构建结果、串口取证点、逐 Issue 场景结果）落在固件功能规格文档 §8.5，含**强制脱敏**：证据不得含用户原始任务/日程文字或任何凭据。§4.22/§8.9，固件规格 §8.4/§8.5 |
...
> **⚠️ v2.15.0 迁移状态：本帧是旧通路的 App 侧半边，正在被 `0x23` 任务库取代。** 固件在 **B 阶段**（Issue #26）停发入站 `0x10 EnterTaskIn` 后就**不会再收到本帧**——任务详情与三阶段文案改从本地已提交任务库读取（§4.22）。App 在 B 阶段仍保留生成与回发能力作为安全网，**C 阶段**（Issue #29）才删除。**固件不要为本帧新增功能**；三阶段表见 §4.22「迁移与切换阶段」。
...
> **⚠️ v2.14.0：本帧已升级为每个连接的强制前置握手。** App 在安全握手完成后立即发送本帧，并在收到并处理完对应 `0x21` 之前**不发送任何 presentation 数据**；**15 秒**未完成即判本次连接失败并主动断开。**没有离线事件时也必须回空批次**，否则设备会陷入「连上就断、反复重连」。完整契约（持久化、原始顺序、幂等身份、回放优先）见 **§5.23**。
...
> ⚠️ **先分清方向双义（§2.4）**：`0x10` **出站（App→Device）= DayPack**，**入站（Device→App）= EnterTaskIn**。本迁移停用的是**入站的 `0x10 EnterTaskIn`** 与它触发的**出站 `0x11 TaskInPage`**；出站 DayPack 走的是另一条独立迁移（由 `0x24` 接管），**不在本表范围内，别一起砍**。

| 阶段 | App | 固件 | 进入条件 |
|------|-----|------|---------|
| **A（当前，v2.14.0）** | 双通路：既发 `0x23` 任务库，也在收到入站 `0x10 EnterTaskIn` 时生成并回 `0x11 TaskInPage` | 走旧通路：进任务发入站 `0x10`、等 `0x11` | — |
| **B（固件切换，Issue #26 交付）** | **不变**——仍保留回 `0x11` 的能力，作为过渡期安全网 | **停发入站 `0x10 EnterTaskIn`**；进任务直接读本地已提交任务库的标题、详情与三阶段文案，按本地专注分钟选句 | 固件完成 #26 并通过真机验收 |
| **C（App 清理，Issue #29）** | **删除 `0x11 TaskInPage` 的生成与发送路径**；入站 `0x10` 不再触发现场生成 | 不变（B 阶段已停发） | B 阶段真机验收通过 |

1. **B 阶段是固件单方切换，不需要 App 同步发版。** App 在 B 期间仍能回 `0x11`，因此固件切换若出问题可以**单方回退到 A**，不会把设备变砖。这是本设计刻意留的安全边界。
2. **C 阶段必须等 B 真机验收通过**——这正是 Issue #29 标注 `Blocked by #26/#27/#28` 的原因。C 一旦发布，任何仍在发入站 `0x10` 的固件进任务只会得到**空白页**（App 不再回 `0x11`），且没有错误提示。
3. **任务库缺失时不得回落旧通路。** 设备从未成功提交过任务库（首次绑定、库丢失、校验失败）时，B 阶段之后应**显示空任务列表 + 固件本地默认对话**，并按 §5.8 报告 missing 让 App 补发全量库；**不得**改回发入站 `0x10` 索取现场详情。理由：回落会让「这台设备是否已迁移」变成运行期不确定状态，C 阶段就无法安全执行。
4. **`0x1B TaskListSnapshotAck` 不在本次迁移范围内。** 它是 Complete/Skip/Refresh 的业务确认（v2.9.0），三个阶段中**一直生效**，Issue #29 的删除范围**不含**它。`0x23` 管任务详情与三阶段文案，`0x1B` 管动作确认与 Overview 清单，两者职责不同、不可互相替代。
...
> **⚠️ v2.15.0 迁移状态：本帧属于迁移 A 阶段的旧通路。** 固件切到 **B 阶段**（Issue #26）后**停发本帧**——短按直接读本地已提交任务库（§4.22）并立即进入专注页，**不发 `0x10`、不等 `0x11`、不设超时**，离线在线行为一致。本节的「短按/等待边界」5 秒规则**只在 A 阶段有效**，B 阶段之后整段作废。App 在 B 阶段仍保留回 `0x11` 的能力作为安全网，直到 **C 阶段**（Issue #29）才删除。三阶段表与切换条件见 **§4.22「迁移与切换阶段」**。
...
> **⚠️ v2.15.0：启动触发点随迁移阶段变化。** A 阶段由「收到 `0x11 TaskInPage` 建立任务上下文」触发；**B 阶段起固件不再收到 `0x11`**，改为「设备本地从已提交任务库进入队首任务的专注页」时建立上下文并启动心跳。**切到 B 后若仍把心跳绑在 `0x11` 上，心跳将永不启动，息屏期间能量瓶停更**（与 v2.5.25 §8.7 问题 5 同类的静默失效）。见 §4.22「迁移与切换阶段」。
...
### 5.23 离线任务动作：持久化、幂等回放与回放优先（v2.14.0，wire 不变）
...
### 6.8 顺序任务队列、来源页面与静态反馈（v2.14.0，wire 不变）
...
### 8.8 离线动作与 App 最新状态的冲突合并（v2.14.0，wire 不变）
...
### 8.9 设备侧持久化与断电恢复（v2.15.0，wire 不变）
...
### 9.6 断联专注连续性与当前任务编辑锁（v2.14.0，wire 不变）
...
| 固件功能规格文档.md | v1.11.1 | 固件功能规格；「实施总清单」第五、六组 = Issue #26/#27/#28 的固件视角待办；§8.4 验收清单、§8.5 交付证据与「静态反馈专项取证」 |
