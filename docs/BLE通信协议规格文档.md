...
**版本:** v2.7.1
**更新日期:** 2026-07-23
**状态:** v2.7 自定义形象事务协议待固件联调。`0x15` 只暂存候选 KRI，`0x22` 负责校验结果、原子提交、精确擦除和恢复查询；App 不再把 GATT 写回执当作设备持久化成功。v2.7.1 补充 Apple 节日/节气过滤与无中文字库时的标题兜底，wire 字节不变。电子墨水屏五页结构继续按 v2.6.0 执行。
...
| v2.7.1 | 2026-07-23 | **Apple 日历参考项过滤与无中文字库标题兜底（纯 App 行为，wire 字节不变）**：生日历、订阅来源中的节日/节气日历不进入 App 日程与 DayPack，普通课程/团队/赛事订阅继续同步；用户原始中文保留在 App。硬件标题净化后无可读 ASCII 时，事件发 `Calendar Event`、任务发 `Task`、FirstUp 发 `Next item`（定时项保留时间前缀，如 `09:30 Next item`），避免空标题。§3.5/§4.3/§4.4/§4.7/§4.8/§4.11 |
...
- **字符集：仅可打印 ASCII `0x20`–`0x7E`（v2.5.14 新增，App 侧保证）。** App 在编码边界（`appendString` 单一咽喉点）统一净化所有出站字符串字段，**保证只含可打印 ASCII**。非 ASCII 字符——LLM 常吐的弯引号 `’ ‘ “ ”`、长破折号 `— –`、省略号 `…`、不间断空格；用户手输或日历同步带入的 emoji、重音字母、CJK 等——在下发前被**转写为 ASCII 近似**（弯引号→`'`/`"`、破折号→`-`、`…`→`...`、`café`→`cafe`、全角→半角）或**直接丢弃**（emoji / CJK / 货币 / 数学符等无 ASCII 近似者）。若日程标题、任务标题或下一项标题净化后为空，App 分别下发 `Calendar Event`、`Task`、`Next item`，不允许硬件收到空标题；用户原始中文仍保留在 App 数据中。Apple 的生日历，以及订阅来源中的节日/节气日历在同步入口过滤，不进入 DayPack；普通课程、团队、赛事订阅仍会同步。因此**固件收到的任何文本字段都是纯可打印 ASCII，无需处理多字节 UTF-8 排版字符，也不会出现「豆腐块」`□`**。此为 App 侧保证，wire 格式不变（ASCII 是 UTF-8 子集，`Length` 仍是字节数）；固件字库只需覆盖 `0x20`–`0x7E`（见 §6.6 缺字回退已降级为兜底安全网）。
...
| Offset | Field       | Size        | Max Length | 描述 |
|--------|-------------|-------------|------------|--------------------------|
| 0      | Title       | 1 + N bytes | 30 bytes   | 任务标题；净化后为空则为 `Task` |
...
| Offset | Field     | Size        | Max Length | 描述 |
|--------|-----------|-------------|------------|--------------------------|
| 0      | Title     | 1 + N bytes | 25 bytes   | 事件标题；净化后为空则为 `Calendar Event` |
...
| ...    | FirstUp                | 1 + N bytes | 60 bytes   | **下一项（框③）**：「First up:」内容——下一个未来事件「HH:mm 标题」（全天事件仅标题），无未来事件则置顶任务标题，皆无则空串；标题净化后为空则为 `Next item`，定时项保留时间前缀（如 `09:30 Next item`）。App 算好下发（v2.5.8，在 DaySummary 之后，见下注）|
...
| ...    | Title       | 1 + N bytes | 40 bytes   | 事件标题；净化后为空则为 `Calendar Event` |
...
| N+1    | Title          | 1 + N bytes | 30 bytes   | 任务标题；净化后为空则为 `Task` |
...
|--------|----------------------|-------------|------------|--------------------------|
| 0      | TaskId               | 1 + N bytes | 36 bytes   | UUID 字符串 |
| N+1    | TaskTitle            | 1 + N bytes | 40 bytes   | 任务标题；净化后为空则为 `Task` |
...
| 4      | TaskTitle    | 1 + N bytes | 当前任务标题（长度前缀 UTF-8，最多 40 字节；非空原文净化后为空则为 `Task`） |
...
5. `commit`、`eraseExact`、`eraseAll`、`query` 按 OperationID 幂等；重复命令返回已存结果，不重复破坏文件。
...
**幂等：** 固件至少保留当前/最近头像操作的 OperationID 和结果。同 OperationID 重复命令必须返回原结果，不重复 rename、擦除或改激活状态。不匹配的迟到结果不得影响 App 当前操作。
