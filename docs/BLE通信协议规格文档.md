...
**版本:** v2.16.0
**更新日期:** 2026-08-03
**状态:** v2.16.0 **更正 v2.15.0 迁移表的一处设计错误，并宣布 App 已先行落地 Issue #29**（wire 字节仍不变）。

**⚠️ 更正：入站 `0x10 EnterTaskIn` 不删、固件必须继续发送。** v2.15.0 曾写「B 阶段固件停发入站 `0x10`」——**该写法作废**。`0x10 EnterTaskIn` 是 Device→App **唯一**的「专注已开始」信号（§5.2 入站事件表无第二个），停发会让 App 永远不建立专注会话：能量瓶不计、打断检测不启动、`0x14 FocusStatus` 不再下发、专注记录全丢。本次迁移真正废除的是它的**第二个角色**——不再用于索取现场 `0x11 TaskInPage` 详情。

**App 已先行（flag-day）：Issue #29 已落地，App 不再生成也不再发送 `0x11 TaskInPage`**，收到入站 `0x10` 只建立专注会话（会话建立与出站写入结果**解耦**）。固件请按 §4.22「迁移与切换阶段」跟进：照发 `0x10`，发完**立即**从本地已提交任务库进入专注页，**不等 `0x11`、不设超时**。**过渡期靠 §5.3 的 5 秒超时兜底降级（表现为进任务慢 5 秒）——请优先确认该超时确实已实现，未实现会卡在等待页。**

v2.15.0 的其余内容继续有效：设备侧持久化与断电恢复（§8.9，含掉电时机可测矩阵与幂等重提交）、固件交付与验收证据要求（固件规格 §8.5）。
...
| v2.15.0 | 2026-08-03 | **交付硬件团队前的设计补齐（wire 不变；补 v2.14.0 遗留的三项开工缺口）**：① **§4.22 新增「迁移与切换阶段」**——**⚠️ 本项的「B 阶段固件停发入站 `0x10 EnterTaskIn`」已由 v2.16.0 更正作废，勿按本行实现**（`0x10` 是唯一的专注开始信号，停发会让 App 永不建会话）。原文：A（当前双通路）/ B（固件停发**入站** `0x10 EnterTaskIn`、进任务只读本地已提交任务库，Issue #26）/ C（App 删除 `0x11 TaskInPage` 生成路径，Issue #29）三阶段表。**B 是固件单方切换、不需 App 同步发版、可单方回退**；**C 必须等 B 真机验收通过**，否则仍发入站 `0x10` 的固件会拿到空白页且无错误提示。**任务库缺失时显示空态并按 §5.8 报 missing，禁止回落旧通路**（否则「设备是否已迁移」变成运行期不确定状态，C 无法安全执行）。⚠️ 方向双义提醒：本迁移停用的是**入站** `0x10 EnterTaskIn`，**出站 `0x10 DayPack` 由 `0x24` 另行接管、不在本表范围**。`0x1B TaskListSnapshotAck` 三阶段一直生效，**不在 #29 删除范围**。② **新增 §8.9 设备侧持久化与断电恢复**——三个独立持久化对象（任务库 / 当天内容包 / 离线动作）；版本标识与内容必须一起原子落盘（禁止「版本已推进但内容是旧的」）；暂存区与 committed 区物理分离，未校验暂存断电后绝不成为当前版本；附**「掉电时机 × 期望结果」可测矩阵**，其中「提交完成、应答未发出」时 App 会用同版本整包重发，**设备须幂等回 `committed`、不重复擦写 flash**。③ 固件交付与验收证据协议（固件版本、协议版本、构建结果、串口取证点、逐 Issue 场景结果）落在固件功能规格文档 §8.5，含**强制脱敏**：证据不得含用户原始任务/日程文字或任何凭据。§4.22/§8.9，固件规格 §8.4/§8.5 |
| v2.16.0 | 2026-08-03 | **迁移表设计更正 + App 先行落地 Issue #29（wire 不变）**：① **⚠️ 更正 v2.15.0 的错误**：入站 `0x10 EnterTaskIn` **不删、固件必须继续发送**——它是 Device→App **唯一**的「专注已开始」信号（§5.2 入站事件表无第二个），停发将使 App 永远不建立专注会话（能量瓶不计、打断检测不启动、`0x14` 不再下发、专注记录全丢）。迁移真正废除的是它的**第二个角色**：不再索取现场 `0x11 TaskInPage` 详情。§4.22 迁移表按「角色拆分」重写，§5.3/§4.8 就地更正，v2.15.0 的「B 阶段停发入站 `0x10`」写法作废。② **App 先行（flag-day）**：Issue #29 已落地——App 删除 `0x11` 的生成与发送路径，收到入站 `0x10` **只**建立专注会话；**会话建立与出站写入结果解耦**（A 阶段是「`0x11` 写成功才起会话」，现在解析出任务即起）。taskId 解析失败仍回 `DeviceMode(0x12)=Interactive` 解卡且不起会话，§8.7 问题 4 规则不变。③ **固件跟进项（Issue #26）**：照发入站 `0x10`，发完**立即**从本地已提交任务库进入专注页，**不等 `0x11`、不设超时**，收到 `0x11` 也忽略。④ **过渡期风险**：App 已停发 `0x11`，固件若尚未改完，靠 §5.3 的 **5 秒超时 → 本地兜底**降级（进任务慢 5 秒）；**该超时是 v2.10.2 定的规则、固件规格 v1.9.2 起标注仍待实现——未实现会卡在等待页，请优先确认**。§4.8/§4.22/§5.3 |
...
> ⚠️⚠️ **入站 `0x10 EnterTaskIn` 本身不删（v2.16.0 更正）。** 它是 Device→App **唯一**的「专注已开始」信号（§5.2 入站事件表里没有第二个）。固件若停发，App 将永远不建立专注会话——能量瓶不计、打断检测不启动、`0x14 FocusStatus` 不再下发、专注记录全丢。**v2.15.0 的迁移表曾误写为「B 阶段停发入站 `0x10`」，该写法作废，勿按其实现。**
...
| 阶段 | App | 固件 | 进入条件 |
|------|-----|------|---------|
| **A（v2.15.0 及以前）** | 收到入站 `0x10` → 生成并回 `0x11 TaskInPage`，**写成功后**才建立专注会话 | 进任务发入站 `0x10`，等 `0x11`（最多 5 秒，超时本地兜底，§5.3） | — |
...
>
> *(v2.15.0 曾在此写「固件切到 B 阶段后停发本帧」，该写法已作废。)*
