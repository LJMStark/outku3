...
**版本:** v2.18.0
**更新日期:** 2026-08-06
**状态:** v2.18.0 是单一功能版本，修复 `0x1B TaskListSnapshotAck` 在设备重启后版本错位的根因。固件上一个收到的版本是 **v2.17.0**（或 v2.10.2——若 v2.16.0/v2.17.0 尚未施工，请先按 v2.10.2 → v2.17.0 整体差异一次做完，再施工本版）。本版对固件的增量要求：**① NVS 持久化 0x1B 快照版本 12B（Epoch+Revision+CRC32）；② DeviceWake(0x30) payload 从 42B 扩展至 51B，末尾追加 SnapshotState(1B)+SnapshotEpoch(4B BE)+SnapshotRevision(4B BE)**。App 侧已完成；无 wire 格式兼容性破坏。

**本版两件事，都不动 wire 字节、也不新增帧：**

**① 任务与日程各设 20 条上限。** `0x23` 每次事务至多携带当天**前 20 条**任务（按 App 列表顺序，不重排）；`0x24` 至多携带当天**最早 20 条**日程（按开始时间升序）。超出的部分 App 静默丢弃，不做任何提示。**这一条与 v2.16.0 的「不设任何条数上限」相反，旧要求已废除。** 注意作用域：**上限只在 App 发送侧**——固件规则一个字没变，收到多少存多少、不自行计数、不自行截断，放不下仍是整次事务失败回 `capacityExceeded`（§5.21/§5.22）。`UpsertCount` / `EventCount` 仍是 4B，只是取值恒 ≤20。详见 §4.22 / §4.23 / §6.8。

**② BLE 常开，省电由设备侧电源管理与连接参数实现。** 客户要求硬件 BLE **一直保持可连接**（含进入屏保或低功耗后）。**这取消了固件规格 §3.6「进入屏保后断开 BLE 省电」。** 具体做法全在设备侧、**不需要任何自定义帧**：只用 Modem-sleep + 自动 Light-sleep，开齐电源管理相关配置（连接保持中可达 230 µA 量级），并经 L2CAP 申请「短连接间隔 + 高 Peripheral Latency」。应用层**不得**进入「连着但不响应 GATT」的状态；暂时无法执行的操作用 ATT 应用错误码拒绝该次请求。完整要求与推荐取值见 **§2.5**。

> ⚠️ **若你看过任何写着 `0x25 DevicePowerControl`（App 读设备状态 → 下发唤醒 → 轮询等就绪）的中间稿，那套设计已整体撤销，不要实现。** 它的前提不成立：在 ESP32 上**设备能应答 GATT 读，本身就证明应用层已经醒着**；真睡到应答不了时连接早就断了，那帧也发不出去——它想覆盖的「连着但没醒」这个中间态并不存在。字节 `0x25` 回到未分配状态。理由详见 §2.5.3。

**四条最容易踩的，请先看：**

0. **任务库只含当天任务、且至多 20 条**——测试时放未来/无日期任务进 App 后设备收不到**不是 bug**；当天放 25 条任务而设备只收到 20 条**也不是 bug**。无日期任务要上设备，需在 App 里手动设为今天。跨日后旧任务由 App 增量 delete，设备不自行按日期过滤。
...
| **v2.18.0** | 2026-08-06 | **0x1B 快照版本双保险对齐（NVS 持久化 + 0x30 inventory 握手）。** 修复设备重启后 `0x1B TaskListSnapshotAck` 版本号从 RAM 丢失、App 与设备版本永久错位的根因。**设备侧**新增 **NVS 持久化 12B**（Epoch 4B + Revision 4B + CRC32 4B），将最近一次成功应用的 0x1B 快照版本写入 flash；**协议侧** `0x30 DeviceWake` payload 从 **42B 扩展至 51B**（前向兼容追加末尾，旧固件/旧 App 继续用 42B），末尾新增 `SnapshotState(1B)` + `SnapshotEpoch(4B BE)` + `SnapshotRevision(4B BE)` = 9B 库存字段。**App 对齐规则**：`SnapshotState=0x00` 时三字段全为 0，App 清除该设备的本地快照基线并触发立即刷新（下次操作从 epoch=1/revision=1 开始）；`SnapshotState=0x01` 时 Epoch/Revision 非零，App 采纳设备上报版本为新基线（设备 NVS 是权威来源，App 本地不同时以设备为准）；其他组合无效，不改变已知状态。**此机制与任务库库存（§4.22）完全独立、互不影响**。快照 missing 属优先恢复，不受 DeviceWake 10 秒节流阻挡。§5.8 |
| **v2.17.0** | 2026-08-04 | **任务/日程各设 20 条上限；BLE 常开改由设备侧电源管理与连接参数实现。合并发布，不动任何 wire 字节、不新增帧。**　**① 条数上限（推翻 v2.16.0 的「不设上限」）**：`0x23` 至多携带当天**前 20 条**任务（按 App 列表顺序，不重排）；`0x24` 至多携带当天**最早 20 条**日程（按开始时间升序）。超出部分 App **静默丢弃**，无任何 UI 提示。**上限只在 App 发送侧**——固件规则完全不变：收到多少存多少、不自行计数、不自行截断，放不下仍回 `capacityExceeded`（§5.21/§5.22）。`UpsertCount` / `EventCount` 仍是 4B，只是取值恒 ≤20。两个派生现象**不是 bug**：当天 >20 条时被硬件跳过的任务会掉出前 20 而从设备消失（「队尾」即第 21 位）；完成/删除走即时小事务不提拔第 21 条，设备暂时只剩 19 条，最多 180 秒后由完整事务补齐。§4.22/§4.23/§6.8　**② BLE 常开 + 设备侧低功耗（新增 §2.5）**：设备进入屏保或低功耗后必须保持广播与已建立连接可达，**这取消了固件功能规格 §3.6「进入屏保后断开 BLE 省电」**。省电全在设备侧、**不需要任何自定义帧**：只用 Modem-sleep + 自动 Light-sleep（deep sleep 与手动 light sleep 都会断连）；开齐 PM 支持 + `configUSE_TICKLESS_IDLE` + Bluetooth Modem sleep **Mode 1** + light sleep 期间主晶振 + **32.768 kHz 外部晶振**（未开齐时蓝牙持 `ESP_PM_NO_LIGHT_SLEEP` 锁、根本进不了光睡眠——「开 BLE 就耗电」通常是这个，不是物理限制），达成后连接保持中约 **230 µA**（Modem sleep 单独约 17.9 mA）；经 L2CAP 申请「短连接间隔 + 高 Peripheral Latency」（跳过连接事件是协议许可、不触发 supervision timeout，有数据时下个事件即可响应），申请值须满足 Apple 六条约束（§2.5.2），推荐 Interval 15–30 ms / Latency 20 / Supervision 6000 ms。**应用层不得进入「连着但不响应 GATT」的状态**——暂时不能执行的操作用 **ATT 应用错误码 `0x80`–`0x9F`** 拒绝该次请求，App 会退避重试；墨水屏保持画面不耗电，没有理由为省电拆掉业务状态机。　**⚠️ 中间稿里的 `0x25 DevicePowerControl` 唤醒握手已整体撤销，不要实现**：它用 GATT 读探测「应用层是否醒着」，而能应答 GATT 读本身就证明应用层醒着——探针与被探测属性是同一件事；真睡到应答不了时连接已断，那帧也发不出去。字节 `0x25` 回到未分配状态，理由详见 §2.5.3。App 侧连接预算维持 30 秒（connect 10s + 安全握手 5s + 回放 15s）；`0x21` 回放屏障的 fail-closed **保持不变**——那道门槛有状态正确性理由，且是链路上唯一的硬门槛。 |
| **v2.16.0** | 2026-08-03 | **设备离线自持：完整任务库、当天内容包、断联专注与离线动作（合并发布）。** 本版**合并 2026-08-03 当天的全部内部迭代 v2.11.0–v2.15.0**——那些编号**未曾对固件单独发布**，App 代码注释与内部记录里的 v2.11.x–v2.15.x 标号与本版同义，无需逐版对照。**上一个对固件发布的版本是 v2.10.2**，请按 v2.10.2 → v2.16.0 的整体差异施工。**本版不改任何已有帧的字节布局**；新增两个双向帧，其余全部是设备行为变化。　**① 新增 `TaskLibraryTransaction(0x23)`**：可原子提交的完整/增量设备任务库。携带非零 Epoch/Revision、**当天任务**（08-04 修订：dueDate 当天 ∪ 手动设为今天的无日期任务；未来/过期/未选入的无日期任务不发。**仍不设八项或其他任意条数上限**——收窄的是日期范围，不是数量。**⚠️ 本句已被 v2.17.0 推翻：App 侧现至多发当天前 20 条，见 §4.22；固件侧「不自行截断」的规则不变**）、每条任务的标题+详情+三条阶段文案，以及 CRC-32/IEEE。设备完整重组、严格校验后才**原子替换**已提交库，再回同 type 结果；中断或失败**不得改动旧库**，容量不足回 `capacityExceeded` 且**禁止截断后报成功**。普通内容变化走 **180 秒稳定窗**（断连/重连不重计），完成动作立即生效不等窗口。已有有效库时只发 upsert/delete 增量，Base 不匹配回 `baseMismatch`。§4.22/§5.21　**② 新增 `DailyContentTransaction(0x24)`**：当天日程与全部每日文案的原子完整替换，与任务库**使用独立版本和持久化区**，改日程不触发任务库重传。只纳入设备本地当天日程（未来日程不传；**当天条数上限见 v2.17.0：App 至多发最早 20 条**）；早间/空闲/收尾、逐日程文案、屏保金句、总结点评与结尾金句**全部本地选择，进页面不现场请求**。**跨自然日立即失效**：日期变化后昨日程与每日文字不可读，页面显示新日期+空日程+本地默认对话；**任务库由 App 同一轮按新日重算重发（08-04 修订，§4.22 规则 11——设备旧库在新版到达前继续可用），进行中的专注不受影响**。§4.23/§5.22　**③ 断连不结束专注**：设备保持专注页、按本地时钟继续计时并按 `0–5 / 6–15 / 16+` 本地切换三阶段文案；App 保留同一会话。重连**不切页、不重建、不清零**，并恢复 §5.7 周期唤醒。**v2.5.25 的「断连即会话结束 → 退回概览」就此作废**（§5.7、§8.7 问题 5 已就地标注）。连接中且正在专注时 App 拒绝编辑该任务；断连后可编辑，但重连不替换正在显示的专注页，退出后再次进入才生效。跳过不发放能量瓶。§9.6　**④ `0x20 EventLogRequest` 升级为每连接强制前置握手**：App 在处理完对应 `0x21` 之前**不下发任何显示数据**，**15 秒未完成即判连接失败并主动断开**。**⚠️ 没有离线事件时固件也必须回一帧空 `0x21`**——不回不产生任何错误码，只表现为「连上就断、反复重连」。离线完成/跳过须**先本地生效**并跨断电持久化非零 OperationID、payload 与**原始发生顺序**（不得按 RTC 重排）；重发原样复用同一 OperationID 与 payload。回放严格先于 App 下发。§4.13/§5.23　**⑤ 顺序任务队列、来源页面与静态反馈**：短按只进队首，完成移除、跳过移队尾且**不加奖励**；无滚轮/高亮/任意选择，App 是调序唯一入口；显示条数与保存条数分离。专注/每日总结/屏保各自**返回进入前的来源页面**。墨水屏只做**静态刷新**——无声音、无震动、无动画（不约束 iPhone 端）。§6.8　**⑥ 冲突合并四条固定优先级**：完成 > 内容编辑；跳过与内容编辑并存（最新内容 + 队尾）；删除 > 跳过；先取 App 最新顺序再按离线跳过先后移队尾。断联专注的任务被删除时重连**不打断**，完成后时长与能量照常结算但任务不恢复。§8.8　**⑦ 迁移与切换阶段**：废除的是 `0x11 TaskInPage` 现场详情应答——**App 已停止回发**。**入站 `0x10 EnterTaskIn` 必须继续发送**（Device→App 唯一的「专注已开始」信号，停发会让 App 永不建立会话：能量瓶不计、打断检测不启动、`0x14` 不再下发）。固件改为：照发 `0x10`，发完**立即**从本地已提交任务库进专注页，**不等 `0x11`、不设超时**。**⚠️ 过渡期**：App 已停发 `0x11`，固件改完前依赖 §5.3 的「等 5 秒 → 本地兜底」降级——**有兜底＝慢 5 秒，没兜底＝卡在等待页，请优先确认该超时是否已实现**。任务库缺失时显示空态并按 §5.8 报 missing，**禁止回落**旧通路。`0x1B TaskListSnapshotAck` **不在废除范围**，一直生效。§4.8/§4.22/§5.3　**⑧ 设备侧持久化与断电恢复**：任务库/当天内容包/离线动作**各自独立**持久化；**版本标识与内容必须一起原子落盘**（禁止「版本已推进但内容是旧的」）；暂存区与 committed 区**物理分离**，未校验暂存断电后绝不能成为当前版本；重启读最后一份 committed，没有就如实报 missing。**「提交完成但应答未发出」时 App 会用同版本整包重发，设备应幂等直接回 `committed`、不重复擦写 flash**。附「掉电时机 × 期望结果」可测矩阵。§8.9　**⑨ 交付与验收证据**：固件版本、对齐的协议版本、构建结果、设备型号、串口取证点与逐条场景结果，**含强制脱敏**（不得含用户原始任务/日程文字或任何凭据）。详见固件功能规格文档 §8.4/§8.5。　**⚠️ 若你在 2026-08-03 当天看到过写着「B 阶段停发入站 `0x10 EnterTaskIn`」的中间稿，那句话是错的**，以本版 ⑦ 与 §4.22 为准。 |
...
### 2.5 BLE 常开与设备低功耗（v2.17.0 新增）

客户要求：**硬件 BLE 一直保持可连接**，进入屏保或低功耗后广播与已建立的连接都必须继续可达。
（本条取消固件功能规格 §3.6 的「进入屏保后断开 BLE 省电」。）

**省电完全在设备侧解决，协议层不新增任何帧、App 侧不参与。** 曾有中间稿定义 `0x25 DevicePowerControl`
让 App 显式唤醒设备，该设计已整体撤销——理由见本节末尾。

#### 2.5.1 固件必须满足的三条

1. **不得进入会断连的睡眠。** ESP32-S3 上 deep sleep 与手动 light sleep 都会掉射频、断开连接
   （deep sleep 的唤醒等同重启）。**唯一可用的是 Modem-sleep + 自动 Light-sleep**——该模式下
   BLE controller 按连接事件调度自动唤醒系统，上下文完整保留、醒来从原地继续。

2. **按下表配置，才真的能省电。** 未启用下列支持时，蓝牙会持有 `ESP_PM_NO_LIGHT_SLEEP` 锁，
   设备进不了 light sleep——**「开着 BLE 就很耗电」通常是这几项没配，不是物理限制**。
   ESP-IDF 官方的 NimBLE power_save 示例给出了 ESP32-S3 **在 BLE 连接保持中**的光睡眠实测电流，
   证明「连着 BLE + 光睡眠」是官方支持且有示例的组合。

   | 配置项 | 要求 |
   |--------|------|
   | 电源管理（PM）支持 | 开启 |
   | `configUSE_TICKLESS_IDLE` | 开启 |
   | Bluetooth Modem sleep | **Mode 1** |
   | Light sleep 期间主晶振 | 按需开启 |
   | 低功耗时钟源 | **32.768 kHz 外部晶振**（用内部 RC 会因时钟精度不足导致连接不稳；BLE 规范要求 ±500 PPM） |

   达成后的实测量级：Modem sleep ≈ 17.9 mA；**Modem + 自动 Light sleep + 32 kHz 晶振 ≈ 230 µA**（连接保持中）。

3. **应用层不得进入「连着但不响应 GATT」的状态。** 设备既然维持着连接，就必须能响应 ATT 请求。
   墨水屏保持画面本身不耗电，没有理由为省电把业务状态机拆掉——230 µA 已经足够低。
   若某个操作确实暂时无法执行（如 EPD 正在刷新），用 **ATT 应用错误码（`0x80`–`0x9F`）拒绝该次请求**，
   App 会退避重试；**不要**用自定义握手帧表达「我没准备好」。

#### 2.5.2 连接参数：用 Peripheral Latency 省电

设备应通过 **L2CAP Connection Parameter Update Request** 主动申请省电参数。
**Peripheral（Slave）Latency 正是为此设计的**：允许 peripheral 连续跳过 N 个连接事件不应答，
**协议明确许可、不会触发 supervision timeout**；而一旦有数据要发，下一个连接事件就能立即响应，
不必等跳过额度用完。典型用法是「短连接间隔 + 高 latency」——空闲时省电，有事时低延迟。

iOS 会拒绝不合规的参数，申请值**必须**同时满足（Apple 加速器设计准则）：

```
Interval Min ≥ 15 ms，且为 15 ms 的整数倍
Interval Min + 15 ms ≤ Interval Max
Peripheral Latency ≤ 30
Interval Max × (Latency + 1) ≤ 2 秒
2 秒 ≤ Supervision Timeout ≤ 6 秒
...
| 参数 | 值 | 校验 |
|------|-----|------|
| Connection Interval | 15–30 ms | 30 = 15 × 2 ✓ |
| Peripheral Latency | 20 | ≤ 30 ✓ |
| Supervision Timeout | 6000 ms | 30 × 21 = 630 ms ≤ 2000 ✓；630 × 3 = 1890 < 6000 ✓ |

大数据量传输（`0x15` 头像帧、多分片 `0x23`/`0x24`）期间可临时申请更激进的参数，传完再申请回省电档。

#### 2.5.3 为什么撤销 `0x25`

被撤销的唤醒握手是「App 读设备状态 → 若休眠则下发唤醒 → 轮询等就绪 → 读到就绪才通信」。
这个设计有一个无法绕开的缺陷：**它用 GATT 读来探测「应用层是否醒着」，而能响应 GATT 读本身
就证明应用层醒着**——探针和被探测的属性是同一件事。于是只有两种结局：

- Query 有回复 → 设备已就绪 → 握手多余；
- Query 无回复 → 连接已断（deep sleep / 掉线）→ 握手根本无从执行，App 走的是重连路径。

它想覆盖的「连着但没醒」这个中间态，在 ESP32 的架构下不存在：GATT 回调运行在主 CPU 的 host 栈上，
设备能回一帧就意味着射频、controller、host task、应用回调**四层全醒**。

此外该设计还有三处与 BLE 惯例相悖，一并记录以免重蹈：
① 轮询读取违反「优先订阅通知而非反复读取」的平台指引，且每次连接都额外消耗双方射频；
② 500 ms 轮询间隔约为一次 ATT 往返（30–90 ms）的十倍，是空转不是采样；
③ 令其 fail-closed 会在连接链路上串联第二道硬门槛，失败率相乘，并与「连接尝试不会自动超时」
叠加成热重连循环。**`0x21` 回放屏障的 fail-closed 不受此影响**——那道门槛有状态正确性理由
（不消费完离线事件就下发会把设备刷成错误状态），且是链路上唯一的硬门槛。
...
**App 响应：**
- **不回发 `0x11 TaskInPage`**（v2.16.0 已废除，见上方迁移状态与 §4.8）；设备从本地已提交任务库（`0x23`）读取详情与阶段文案
- 记录该任务的专注会话开始时间戳，据此建立专注会话
...
| 42     | SnapshotState | 1 byte | **v2.12.0：** `0x00`=快照版本缺失（RAM 已清零），`0x01`=有已应用版本 |
| 43     | SnapshotEpoch | 4 bytes | Big Endian；缺失时填 0，已提交时必须非零（来自 NVS 持久化） |
| 47     | SnapshotRevision | 4 bytes | Big Endian；缺失时填 0，已提交时必须非零 |

> **固件版本要求：** v2.12.0 固件的实时 DeviceWake payload 固定 **51B**；v2.11.1 固件为 42B；v2.7 固件为 29B。App 仍能读取旧 0/1/4/9/29/42B 唤醒包，但只有 51B 包能报告 `0x1B` 快照版本库存。版本语义仍是本次启动实际运行的固件版本。
>
...
>
> **NVS 持久化要求（v2.12.0 固件新增）：** 设备须将最近一次成功应用的 `0x1B` 快照版本（Epoch 4B + Revision 4B + CRC32 4B = 12B）写入 NVS，在重启/断电后可读出。`DeviceWake` 上报时从 NVS 读取：有记录则 `SnapshotState=0x01`，否则（首次启动/flash 清空）`SnapshotState=0x00` 且三字段填 0。

**App 响应：**
1. 若 payload 非空：更新并显示设备电量。
2. 若 payload ≥ 4 字节：记录固件版本（Settings 显示；OTA 升级前后对比判定升级结果，见 §4.17）。
3. 若 payload = 29B、42B 或 51B：记录 committed AvatarID+长度+CRC。存在 `PendingCustomAvatarOperation` 且当前没有活跃事务时，只触发一次优先恢复；App 随后发送 `0x22 query` 并按 §4.12 的三分支处理。DeviceWake 不含 `CustomActive`，不得直接提交候选身份。活跃传输期间不得让 DeviceWake 抢占事务；无待办操作时仅记录库存，不启动后台重发。
4. 若 payload = 42B 或 51B：`TaskLibraryState=0x00` 时四个版本/CRC 字段必须全为 0，App 立即安排完整任务库；`TaskLibraryState=0x01` 时 Epoch/Revision 必须非零。App 已有该设备本地绑定记录时更新精确 committed 状态并阻止无条件全量重发；没有本地绑定记录时仍视为首次绑定并发送完整任务库，避免采信其他账号或旧 App 遗留内容。其他组合视为无效库存，不改变已知状态。
5. **若 payload = 51B（v2.12.0 双保险对齐）：** `SnapshotState=0x00` 时三个版本字段必须全为 0，App 清除该设备的 `0x1B` 版本基线并触发立即刷新（下次操作从 epoch=1/revision=1 重新开始）；`SnapshotState=0x01` 时 Epoch/Revision 必须非零，App 采纳设备上报版本作为新基线（若 App 本地记录与设备不同，以设备为准——设备 NVS 是权威来源）。其他组合视为无效库存，不改变已知状态。此机制与任务库库存独立，互不影响。
6. 同步时间并发送更新数据。任务库 missing 或快照版本 missing 属优先恢复，不受普通 DeviceWake 10 秒节流阻挡。
...
- 用户在设备上选中任务并进入详情页时，固件必须发送 EnterTaskIn (0x10)——它是 App 建立专注会话的唯一信号；App **不回发 `0x11`**（v2.16.0 已废除，见 §5.3），设备从本地已提交任务库（`0x23`）读取详情。
...
| 固件功能规格文档.md | v1.13.1 | 固件功能规格；「实施总清单」第五、六组 = Issue #26/#27/#28 的固件视角待办；§8.4 验收清单、§8.5 交付证据与「静态反馈专项取证」 |
| BLE初次联调指南.md | v0.2.3 | 严格请求、`0x1B` 业务确认与 `0x10 → 0x1B` 时序验收 |
