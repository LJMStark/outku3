...
```
 ...
 sending（发送 0x18，等待最多 5 秒）
   ...
   │
   └── 5 秒无应答，且连接仍存活：
         累计尝试 < 3 次（含首次发送） → 重发 0x18，回到 sending 重新等待
         累计已尝试 3 次              → failed(noResponse)
```
...
- 升级触发入口在有效的专注会话进行中时禁用（避免升级导致的断连被误判为专注会话异常中断）。

**已知边界（待与固件确认）：** 若升级写入或新固件启动失败，设备按 §5「安全与可靠性」约定自动回滚至旧固件并重启——此时 App 侧观察到的现象（断连 → 等待 → 收到 `DeviceWake`）与升级成功**完全一致**。协议当前**没有**让 App 区分"已升级到新版本"与"已回滚回旧版本"的机制（`DeviceWake` 及其他帧均不含固件版本号字段）。当前处理：App 只要收到 `DeviceWake` 就视为流程结束、不再提示错误（即便实际是回滚），不阻塞本轮功能，但建议后续与固件确认是否需要在 `DeviceWake`（或紧随其后的一帧）中带上固件版本标识，让 App 能真正确认升级是否成功，而不是仅凭"连接恢复了"就假定成功。
