...
**版本:** v2.17.0
**更新日期:** 2026-08-04
**状态:** **本版在 v2.16.0 基础上新增一个帧、收紧两条业务规则。** 固件上一个收到的版本是 **v2.10.2**；若你尚未施工 v2.16.0，请按 v2.10.2 → v2.17.0 的整体差异一次做完。v2.16.0 的合并说明仍然有效：那批 v2.11.0–v2.15.0 的内部编号未曾单独发布，**请不要逐版对照**。
...
**2026-08-04 修订（本版新增）：`0x25 DevicePowerControl` 与 BLE 常开。** 客户要求硬件 BLE **一直保持可连接**（含进入低功耗后），App 在下发任何业务数据前先查询设备功耗状态，若处于低功耗则先发唤醒命令并等待设备回到 active。新增双向帧 `0x25`，见 §4.24 / §5.24。**这条取消了固件规格 §3.6「进入屏保后断开 BLE 省电」**——续航影响需硬件团队重新评估。

**本版不改任何已有帧的字节布局。** 新增一个双向帧（`0x25` 设备功耗控制），两条上限规则不动 wire 结构（`UpsertCount` / `EventCount` 仍是 4B，只是取值恒 ≤20）。

**五条最容易踩的，请先看：**

-1. **`0x25` 无应答不会导致连接失败。** App 侧 fail-open：查询超时（2 秒）或唤醒超时（3 秒）都当作设备醒着继续下发数据。尚未实现 `0x25` 的固件**不需要**为了让 App 连上而紧急施工——但每次连接会白等 2 秒。这与紧邻的 `0x20` 屏障**方向相反**（那个是 fail-closed，见下面第 1 条），不要混为一谈。
...
| **v2.17.0** | 2026-08-04 | **任务/日程各设 20 条上限；新增 `0x25 DevicePowerControl` 与 BLE 常开要求。**　**① 条数上限（推翻 v2.16.0 的「不设上限」）**：`0x23` 至多携带当天**前 20 条**任务（按 App 列表顺序，不重排）；`0x24` 至多携带当天**最早 20 条**日程（按开始时间升序）。超出部分 App **静默丢弃**，无任何 UI 提示。**上限只在 App 发送侧**——固件规则完全不变：收到多少存多少、不自行计数、不自行截断，放不下仍回 `capacityExceeded`（§5.21/§5.22）。`UpsertCount` / `EventCount` 仍是 4B，只是取值恒 ≤20。两个派生现象**不是 bug**：当天 >20 条时被硬件跳过的任务会掉出前 20 而从设备消失（「队尾」即第 21 位）；完成/删除走即时小事务不提拔第 21 条，设备暂时只剩 19 条，最多 180 秒后由完整事务补齐。§4.22/§4.23/§6.8　**② 新增 `0x25 DevicePowerControl`（双向）**：App 发 1B 命令（`0x00` Query / `0x01` Wake），设备回 3B（CommandEcho + PowerState + Status）。每次连接在**安全握手之后、`0x20` 之前**发一次；设备回 lowPower 时 App 再发 Wake 并等它回到 active。**顺序不能颠倒**——`0x20` 屏障是 fail-closed（15 秒判失败断开），把唤醒放在它之后等于每次连接都失败。**本帧全程 fail-open**：Query 超时 2 秒、Wake 超时 3 秒，超时/格式非法/设备拒绝一律当作设备醒着继续下发，**所以尚未实现 `0x25` 的固件不会连不上**，只是每次连接白等 2 秒（不认识的 Type 静默丢弃即可，不要回错误帧）。CommandEcho **必须回显**，否则 Query 的迟到应答会被误当成 Wake 成功。`0x25` 不得写入 `0x21` 批次。App 侧连接预算同步放宽到 40 秒。§4.24/§5.24　**③ BLE 常开**：设备进入低功耗后必须保持广播与已建立连接可达。**这取消了固件功能规格 §3.6「进入屏保后断开 BLE 省电」**，续航影响（原承诺待机 4寸 ~30 天 / 7寸 ~20 天）需硬件团队重新评估并回报。 |
...
| 命令 | 语义 |
|------|------|
| `0x00` Query | **只查询，不改变设备状态。** 设备立即回一帧 §5.24，报告当前 PowerState。 |
| `0x01` Wake  | 请求退出低功耗。设备准备就绪后回一帧 §5.24，PowerState 必须为 `active`。 |
...
1. 每次连接在**安全握手完成之后、`0x20 EventLogRequest` 之前**发一帧 `Query`。
2. 若设备回 `PowerState=lowPower`，App 紧接着发一帧 `Wake`，等设备回到 `active` 后才继续。
3. 无论上述结果如何，随后照常进入 `0x20 → 0x21` 强制回放握手（§4.13 / §5.23）。

> **顺序不能颠倒。** `0x20` 屏障是 fail-closed——15 秒收不到 `0x21` 就判连接失败并主动断开。设备若在低功耗态处理不了 `0x20`，把唤醒放在它之后等于每次连接都失败。

**固件实现要求：**

1. **Wake 的应答必须在设备真正能接收业务帧之后才发。** 不得「收到就回 `active`，然后再慢慢醒」——App 收到应答后会立刻开始下发 `0x20`、`0x23`、`0x24`、DayPack。
2. 无法唤醒时回 `PowerState=lowPower` + 非零 Status，不要不回。
3. `0x25` 是连接内实时控制帧，**不得写入、排队或重放到 `0x21 EventLogBatch`**（与 `0x19`/`0x1A`/`0x22`/`0x23`/`0x24` 同类）。
4. 设备也可以**主动**上报功耗状态（例如即将进入低功耗），用 `CommandEcho=0xFF`，见 §5.24。

**App 侧超时与 fail-open（重要，决定固件的排期压力）：**

| 阶段 | 超时 | 超时后行为 |
|------|------|-----------|
| Query | 2 秒 | 记日志，**当作设备是醒的**，跳过 Wake 直接进入 `0x20` |
| Wake  | 3 秒 | 记日志，**当作设备是醒的**，继续进入 `0x20` |

> **本帧全程 fail-open，与 `0x20` 屏障的 fail-closed 刻意相反。** 丢失离线动作会造成数据错乱，所以 `0x20` 必须严格；而「查不到功耗状态」的正确降级就是继续——设备至少已经维持着 BLE 连接。因此：
>
> - **尚未实现 `0x25` 的固件不会连不上**，只是每次连接白等 2 秒（不认识的 Type 静默丢弃即可，不要回错误帧）。
> - 写失败、应答格式非法、设备明确拒绝，一律同样处理：记日志后继续。
>
> App 侧的整条连接预算已同步放宽到 40 秒（connect 10s + 安全握手 5s + 功耗握手 ≤5s + 回放 15s）。
...
**CommandEcho 必须回显，不能恒填 0。** 一次连接会连发 Query→Wake 两条，若不回显，Query 的迟到应答会被 App 当成 Wake 的应答、误判设备已醒并立刻开始下发数据。App 侧按回显值精确匹配，回显不符的一律丢弃。
...
**App 侧处理：** 全程 fail-open。格式非法（长度 ≠3 或 PowerState 非 `0x00`/`0x01`）、超时、Status 非零，一律记日志后当作设备醒着继续下发数据——详见 §4.24 的超时表。
