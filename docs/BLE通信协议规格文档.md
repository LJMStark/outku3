...
**何时才需要 App 传字体**：仅当字体变为**动态**——用户可在 App 内选字体 / 逐条消息字体不同 / 同一字段运行时换字体。当前是**固定设计映射**，故**不引入字体字节**（YAGNI，同 §6.5 不引入 PanelMode 的同类取舍）。
