...
8. App 按本地日历边界观察自然日变化，并持久保存上次观察日期。前台跨零点、App 从挂起恢复、App 重启或时区变化导致本地日期改变时，昨日程的 180 秒稳定窗和冻结投影立即作废；完整任务库、任务顺序与当前专注资料不得改变。
...
5. 每次读取前先比较设备本地年月日与 committed `Year/Month/Day`。日期不一致时该包仍可保留在原子存储中，但所有页面都必须视为无当天包：显示设备新日期、空日程和固件本地默认对话，不得回退显示昨日文字；`0x23` 任务库和当前专注状态继续可用。
...
- **显示条数与保存条数分离**：屏幕一次显示多少项，与设备实际保存多少任务无关。**不得**因页面布局反过来截断任务库，也不得设置八项或其他任意业务上限（容量不足的正确做法是整次事务失败并报 `capacityExceeded`，见 §5.21）。
