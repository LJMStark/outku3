...
**版本:** v2.19.0
**更新日期:** 2026-08-04
**状态:** **本版收紧两条业务规则（任务/日程各 20 条上限），并把「BLE 常开」的实现方式定为设备侧电源管理 + 连接参数——不新增任何帧。** 固件上一个收到的版本是 **v2.10.2**；若你尚未施工 v2.16.0，请按 v2.10.2 → v2.19.0 的整体差异一次做完。v2.16.0 的合并说明仍然有效：那批 v2.11.0–v2.15.0 的内部编号未曾单独发布，**请不要逐版对照**。
...
**2026-08-04 修订（本版新增）：BLE 常开，低功耗改用链路层标准手段；`0x25` 撤销。** 客户要求硬件 BLE **一直保持可连接**（含进入低功耗后）。**这条取消了固件规格 §3.6「进入屏保后断开 BLE 省电」**。

> ⚠️ **v2.18.0 定义的 `0x25 DevicePowerControl` 唤醒握手已整体撤销，不要实现。** 那套「App 读状态 → 下发唤醒 → 轮询等就绪」的设计前提不成立：在 ESP32 上，**设备能应答 GATT 读，本身就证明应用层已经醒着**；真睡到应答不了时连接早就断了，`0x25` 也发不出去。它想覆盖的「连着但没醒」这个中间态并不存在。省电改由 §2.5 的链路层与电源管理手段解决——**不需要任何自定义帧**。详见 §2.5。

**本版不改任何已有帧的字节布局，也不新增帧。** 两条条数上限不动 wire 结构（`UpsertCount` / `EventCount` 仍是 4B，只是取值恒 ≤20）；`0x25` 已撤销，字节回到未分配状态。
...
### 2.5 BLE 常开与设备低功耗（v2.19.0，取代已撤销的 `0x25`）

客户要求：**硬件 BLE 一直保持可连接**，进入屏保或低功耗后广播与已建立的连接都必须继续可达。
（本条取消固件功能规格 §3.6 的「进入屏保后断开 BLE 省电」。）

**省电完全在设备侧解决，协议层不新增任何帧、App 侧不参与。** v2.18.0 曾定义 `0x25 DevicePowerControl`
...
v2.18.0 的唤醒握手是「App 读设备状态 → 若休眠则下发唤醒 → 轮询等就绪 → 读到就绪才通信」。
