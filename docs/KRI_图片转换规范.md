...
**传输通道（BLE 或 WiFi）与 KRI 编码相互独立**（见 §1.1「转换算法与传输协议是两层独立定义」）。同一份 KRI 既可经 BLE `0x15` 分包传输（4–5 分钟），也可经 **WiFi(SoftAP) HTTP 一次整块上传（秒级，v2.8 起为正式快速通道）**——WiFi 通道见 BLE 通信协议规格文档 §4.20 / §5.20（`0x1A WiFiAvatarSession` 握手）与 `WiFi头像传输协议契约草案.md`（HTTP 端点契约）。App 在传输层计算文件 CRC-32/IEEE（BLE `0x15` v4 帧头与 WiFi HTTP header `X-Kirole-File-CRC32` 均携带），用于定位 BLE/WiFi 分片或整块传输问题。摘要只能作为传输协议元数据，不能追加在 KRI 文件尾部。
