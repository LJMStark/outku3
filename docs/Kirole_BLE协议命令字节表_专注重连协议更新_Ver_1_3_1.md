...
**协议版本:** Ver 1.3.1
**来源:** `docs/Kirole_BLE协议命令字节表_专注重连协议更新_Ver_1_3_1.xlsx`
**转写日期:** 2026-08-19

本文由硬件团队字节表原文件逐页转写，作为 App / 固件联调的字节级参考标准。
语义以 `docs/专注状态重连协议变更_Ver_1_3_1.md` 为准；
裁决表仍见 `docs/Kirole_专注状态重连_App对接说明_Ver_1_3_0.md` §9。
仓库总规格见 `docs/BLE通信协议规格文档.md`，1.3.1 回写见 `docs/BLE通信协议规格文档-v2.13.2补记.md`。
冲突时，**以本表 + 1.3.1 变更说明的固件原文为准**。
...
| 1+ | Tasks[] | 变长 | - | 见「子结构与记录」页 | 任务条目数组；每条 =TaskIdLength(1) + TaskId(N1, N1<=36) + | TitleLength(1) + Title(N2≤30) + IsCompleted(1) |
...
| ... | SettlementReview | 1+N | 180 | 长度前缀字符串 | 每日总结页唯一气泡的完整文案：先总结今天；明天有日历安排时带最早一项，若无日历安排则带优先级最高的未完成明日任务，并给出鼓励；两者皆无时用金句收尾 |
| ... | SettlementQuote | 1+N | 120 | 长度前缀字符串 | 保留字段；App 当前恒发长度 0，固件必须消费该长度字节但不得渲染；DayPack 最后一个字段 |
...
**0x25 OfflineSync 双向协议详解 · Ver 1.3.1（含专注裁决 RESULT）**

**方向：双向｜多字节整数：Big Endian｜FOCUS_RESOLVE(0x06) 必须以 RESULT(0x81) 应答；专注裁决与数据 COMMIT 分离**
...
| 0x25 | 0x81 | RESULT | 设备 → App | 7 | 返回同步处理结果；包含 FOCUS_RESOLVE 应答 |
...
| 13 | DatasetMask | 1 | bit0..bit2 | 本次包含的数据集合 | bit0(0x01)：TaskList \ bit1(0x02)：Schedule \ bit2(0x04)：DayPack | 其他位必须为 0 |
...
| 1..4 | SyncId | 4 | UInt32 BE | 对应同步事务 ID | BEGIN 对应事务 SyncId；FOCUS_RESOLVE 对应 ResolveID |
| 5 | TargetType | 1 | 0x02 / 0x03 / 0x10 / 0x25 | 处理目标类型 | 原命令或 OfflineSync |
| 6 | ResultCode | 1 | 见结果码表 | 同步处理结果 | FOCUS_RESOLVE 仅匹配 ResolveID 的 COMMITTED 可解锁 |
...
| --- | --- | --- | --- | --- | --- |
| 0x00 | ACCEPTED | 已接受请求 | BEGIN / FOCUS_RESOLVE | BEGIN 继续；FOCUS_RESOLVE 不解锁 | ResolveResult=rejected/internalError 时保持状态与队列 |
| 0x01 | STAGED | 数据已暂存 | 0x02 / 0x03 / 0x10 | 等待全部数据后 COMMIT |  |
| 0x02 | COMMITTED | 事务已原子提交 | COMMIT / FOCUS_RESOLVE | COMMIT 完成数据事务；FOCUS_RESOLVE 解除专注锁定 | FOCUS_RESOLVE 相同请求幂等重放仍返回 COMMITTED |
...
**设备离线操作立即生效；FOCUS_RESOLVE 必须收到匹配 RESULT/COMMITTED 后才解锁；DayPack/数据 COMMIT 不是重连必经步骤**
...
| Type 0x25 | Opcode 0x81 | RESULT | Device → App | 7 | SyncId=ResolveID；COMMITTED(0x02) 解锁 |
...
**App → Device｜完成 OP_BATCH 幂等处理后下发；设备返回匹配 ResolveID 的 RESULT/COMMITTED 后，App 才能解除 focusSyncLocked**
...
| 7 | 业务数据变化 | 普通 0x10 只更新后台数据，不退出 TaskIn | 按需发送普通 DayPack；活动专注期间不发 COMMIT | 独立业务同步 | 用 DayPack/COMMIT 驱动 TaskIn 退出 |
